...
> **▶️ RESUME HERE (nav redesign — nutrition, July 2026).** Client nutrition is fully migrated: tabs → `NativeTabs`, headers → `LightHeader`, recipe + meal editors → `(client)` stack routes. Three commits are **pushed** (`895cc07` and earlier): nutrition tabs/headers, round-2 fixes, recipe-nav fix, meal-editor extraction, meal cover-card fix. **⚠️ A further "round-3" batch (items A–C below) is done in the WORKING TREE and committed LOCALLY but NOT pushed** (Vitek asked to skip the push that session) — run `git log origin/main..HEAD` to see the unpushed commit(s); **push them when Vitek confirms the build looks right.**
>
> **NEXT after that: the trainer 5-tab bar → `NativeTabs`** (see the ⬜ items in the nav blockquote), then retire `FloatingTabBar`.
>
...
>
> **Nutrition — round-3 (LOCAL commit, NOT pushed — verify these in the next build):**
