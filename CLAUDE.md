...
> **▶️ RESUME HERE (nav redesign — nutrition, July 2026).** Converted the **client nutrition tabs → `NativeTabs`** + migrated the top-level nutrition headers → glass `LightHeader`, applied **round-2 build-feedback fixes** (content-too-low + hidden `+` buttons + weekly week-switcher), **fixed the recipe-open bug (#1, DONE)**, and **extracted + redesigned the meal editor (#2, DONE)**. Both #1 and #2 are pushed and awaiting Vitek's build verification. **NEXT: the trainer 5-tab bar → `NativeTabs`** (see the ⬜ items in the nav blockquote), then retire `FloatingTabBar`.
> 1. ✅ **DONE — Recipe open/create nav fixed.** Moved `recipe/[id]` + `recipe/create` OUT of the nutrition tabs to `(client)` stack routes (`app/(client)/recipe/…`, parent `(client)/_layout` is a `Stack`), removed the hidden triggers, updated nav paths → `/(client)/recipe/X`, swapped `tabBarH` → `insets.bottom` (full-screen now).
> 2. ✅ **DONE — Meal editor extracted to its own frosted stack route** `app/(client)/meal/[id].tsx` (was a huge in-file overlay in `favourites.tsx`). Vitek approved "all your recommendations": **frosted `LightHeader`** (title = meal name / "New meal"; cover hero scrolls under it, `contentInsetAdjustmentBehavior="never"`); **name is now a body row** (not baked into the header); layout mirrors `recipe/create` — cover → NAME → kcal/P/C/F card → INGREDIENTS (+ Add food) → NOTES → SHARE WITH → **Log this meal** (primary) → Delete. Auto-saves via `savePatch`; **`?isNew=1`** param discards the draft on back if left empty+unnamed. Favourites now `router.push`es to it (`startNewMeal` inserts the draft row then navigates; meal cards navigate; favourites reloads meals `useFocusEffect`). The old overlay + `md` styles + all meal-editor handlers/state were removed from `favourites.tsx`. *(Verify in build: create a meal, add foods, name it, log it, delete; and "make meal from selected foods" via the Foods tab.)*
