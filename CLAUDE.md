...
> **▶️ RESUME HERE (nav redesign — nutrition, July 2026).** Converted the **client nutrition tabs → `NativeTabs`** + migrated the top-level nutrition headers → glass `LightHeader`, applied **round-2 build-feedback fixes** (content-too-low + hidden `+` buttons + weekly week-switcher), and **fixed the recipe-open bug (#1 below, DONE)**. **NEXT:**
> 1. ✅ **DONE — Recipe open/create nav fixed.** `recipe/[id]` + `recipe/create` were `hidden` `NativeTabs.Trigger`s and `router.push` couldn't reach them. Fix applied: **moved both files OUT to `(client)` stack routes** (`app/(client)/recipe/[id].tsx` + `recipe/create.tsx`, parent `(client)/_layout` is a `Stack`), removed the two hidden triggers from `nutrition/_layout.tsx`, updated all nav paths → `/(client)/recipe/X`, and swapped their `paddingBottom: tabBarH` → `insets.bottom` + dropped `useTabBarHeight` (they're full-screen now, no tab bar). recipe/[id] keeps `contentInsetAdjustmentBehavior="never"` (its cover hero fills behind the status bar). *(Verify recipe open + create + edit in the next build.)*
> 2. ⏳ **IN PROGRESS — Meal editor (`mealDetail` overlay in `favourites.tsx`, `md` styles) redesign + frosted header.** Vitek: "meals keeps the old header — it needs to stay frosted see-through, also redesign needed here… we should think how the meal making will look." It's a full-screen in-file overlay (`{mealDetail && …}`) with a dark-green header (tappable name + pencil), cover photo, kcal/P/C/F strip, INGREDIENTS + Add food, NOTES, SHARE WITH pills, Save/Log/Delete. Meals are auto-saved drafts (`startNewMeal` inserts a `saved_meals` row immediately). **It's structurally the twin of `recipe/create` — the open design question (being discussed with Vitek): mirror `recipe/create` exactly (name as a body row, frosted `LightHeader`), and possibly extract it to its own stack route `app/(client)/meal/[id].tsx` (like recipe/create) instead of an in-file overlay.** Do NOT rebuild blind — confirm direction with Vitek first.
>
