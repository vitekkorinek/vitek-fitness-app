...
> **▶️ RESUME HERE — Do Mode HEADER REDESIGN (fixed banner) + colored category cards (July 2026, COMMITTED & PUSHED; REVERSIBLE via a one-line flag).** A long exploratory design session on the **client** Do Mode + workout cards. **All changes are CLIENT-SIDE only — the trainer Do Mode is NOT mirrored yet.** The centerpiece (the fixed header) is gated behind a kill switch **`const FIXED_HEADER = true`** at the top of `app/(client)/workout/[workoutId].tsx`: **flip to `false` + rebuild to return to the exact previous scroll-away header** (the whole old header + nav bar are preserved intact behind `!showFixedHeader`; the main list's `ListHeaderComponent` becomes a `bannerH` spacer when fixed). Plus git history as a second net. Everything JS → hot-reload; typecheck baseline stays **29**. What we did:
> 1. **`components/CategoryCover.tsx` (NEW)** — colored-watermark cover for workout cards: a deep same-hue gradient + a large translucent SF-Symbol watermark (**PLACEHOLDER** icon — meant to become real body-part silhouettes, Push=chest / Pull=back / …). Variants `color` (bright, cards) / `muted` (unused). Only **Push** is wired (`categoryHasCover()` gate + `CATEGORY_GRADIENT`/`CATEGORY_WATERMARK`/`CATEGORY_MUTED_GRADIENT` maps — trivial to extend to the other 8). Used on the client Training-tab **WORKOUTS gallery card + the week-strip session cards** (`app/(client)/(tabs)/train/index.tsx`) — for Push the colored watermark REPLACES the photo (prototype ignores any cover). **Intended end-state = hybrid:** watermark is the free default, a real photo is an opt-in override (NOT built). Non-Push categories are unchanged (still photos).
> 2. **Do Mode expanded-card TIGHTENED** — reclaimed ~45–50px in a 3-set card by trimming gaps/margins only (first-card `marginTop` 12→6, `inlineSetRow` paddingVertical 10→7, kg/reps input padding 8→7, action/bar-selector/start-timer/add-set margins) — **inputs + done-circle kept full-size**. A 6-set exercise still overflows the viewport (expected; scroll inside the rounded card).
> 3. **Expanded-card ACTION BUTTONS reorganized** — the big **Play video / Info** row + divider were REMOVED from the top of the card; replaced by a compact **full-width icon toolbar under the set rows**: `+` (add set/dropset — its inline Add Set/Add Dropset menu got a dismiss **✕** top-right) · 📷 (photo) · ▶ (video) · ⓘ (info, keeps its change-dot). **Start timer stays the big full-width button** below the toolbar (`iconToolbar`/`iconBtn` styles; play icon bumped to size 17 so the filled triangle matches the outline icons).
> 4. **Header timer → Liquid Glass + COLLAPSIBLE** — the white timer/START/FINISH pill is now a **`GlassPill`** (module helper reusing `GlassPanel`). While a session runs it **collapses to a small glass stopwatch** (`timerCollapsed` state, default `true`) → tap to expand to `timer | FINISH` → tap the timer to collapse → tap FINISH to finish (`timerClockGlass` style). Extracted into a shared **`timerControl`** JSX var (used by both the old nav bar and the new banner).
> 5. **FIXED HEADER (option 2) — the big structural change.** A **pinned banner** (height `HEADER_MAX`, same as before) that stays put while the exercise cards scroll underneath. It shows the **ACTIVE exercise's** photo + name + count — image/name/count **follow whichever exercise card you OPEN** (`activeHeaderId`, set in `toggleExpand`; falls back to exercise 1; stays on the last-opened when you collapse). Layout: back `‹` top-left, ⋯ top-right; bottom-left a stack of **overline** (`WORKOUT · Session N · D Mon YYYY` — restored after Vitek noted the session/date went missing) + **big title** (exercise name) + **count** (`N / total`); the **collapsible stopwatch sits bottom-right** (this fixed an earlier "two circles next to ⋯" complaint). Photo source = `activeExercise.extraPhotoUrls[0] ?? thumbnailUrl ?? workout cover ?? CategoryCover watermark ?? gradient`. **Client live sessions only** — past-session view and the entire trainer Do Mode still render the old header. See SPEC §"Do Mode → ⚠️ WIP FIXED HEADER" for the field-level detail.
> - **Vitek's verdict / OPEN next steps (lots deferred — "a lot of work to do"):** he likes the direction; it's a prototype. TODO next sessions: **real body-part watermark icon set** + fill the other 8 categories' gradients; decide the **photo-vs-watermark hybrid** (real photo as an override); **crossfade** between exercise photos (currently a hard cut / pop); consider **scroll-driven** active-exercise (currently expansion-driven — chosen as far safer to build blind); **mirror the whole redesign to the trainer Do Mode**; finalize the header timer's placement ("design better later"). The `PROTO_PUSH_PHOTO` hardcoded URL still lives in the OLD header path (behind the flag) and in the card prototype. **Needs on-device testing of the full session flow** (start → log reps → scroll → open different exercises → keyboard → finish, + a superset workout) — this was built without an intermediate build.
