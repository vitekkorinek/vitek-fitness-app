...
> **▶️ RESUME HERE — Merged Do-Mode preview (real list) + body-silhouette category covers (July 2026, COMMITTED & PUSHED; still WIP, continue next session).** Client-side only so far. Two big threads, both flag-safe and typecheck baseline **29**. Full field-level detail lives in the memory files **[[domode_preview_merge]]** and **[[category_palette_and_covers]]** — read those first next session.
> 1. **Merged pre-session ↔ Do Mode preview — RESTRUCTURED to use the REAL list (no more replica).** The old `components/SessionStartOverlay.tsx` (fake cards) is now orphaned/unused (delete once approved). The REAL client Do Mode list renders read-only inside a **sliding preview panel** built into `app/(client)/workout/[workoutId].tsx`, gated `usePanel = showFixedHeader && previewInitRef.current === true` (**Push launcher only**; flag `MERGED_PREVIEW`; session-intro redirects those taps in). Landing (parked peek) → drag grip up to Review → **Start** runs the lock (panel slides up, banner fades in, handle collapses) into the real running session — same list instance, no handoff. Key pieces: `previewY`/`previewLock`/`previewSpacerH` (all **JS driver only** — mixing native+JS on the panel crashed), an opaque white handle overlay masking scrolled cards, `previewReadOnly` + `previewMode` on ExerciseCard (Review shows the toolbar with **Play/Info active, +/camera dimmed**), keyboard "Done" bar + focused-input auto-scroll (`scrollFocusedInputAboveKeyboard`), and expand→`scrollCardToTop`.
> 2. **Category set + palette + covers reworked.** `lib/workoutCategories.ts`: **added Arms, retired Legs & Recovery** (kept renderable/legacy-mapped; DB rows migrated Legs→Lower Body, Recovery→Mobility). New distinct palette (Push red · Pull blue · Upper Body purple · Arms orange · Lower Body green · Full Body amber · Core pink · Mobility bronze). **`components/CategoryCover.tsx` rebuilt** — the watermark is now a faint **anatomical body silhouette** (via `react-native-body-highlighter`, same as MuscleThumb) with the category's muscles lit (Mobility = soft all-over glow; a foam-roller + a diagonal front/back split for Upper Body were both tried & rejected). `categoryHasCover()` now true for all categories. **My Workouts card (`all-workouts.tsx` `WorkoutItem`) restructured** to cover-on-top + white footer (Done date + ⋯ off the watermark).
> - **Still to continue (next session):** mirror the merged preview + keyboard/scroll niceties to the **trainer** Do Mode; wire the **non-launcher** entries (planned/past/view still use the classic pre-session screen); the **photo-as-override hybrid** (real photo wins on cards when set); per-category cover crop tuning; delete the orphaned `SessionStartOverlay.tsx`; the fixed-header `FIXED_HEADER` kill-switch and `PROTO_PUSH_PHOTO` still exist behind the flag. See SPEC §"Do Mode → MERGED PREVIEW".
