...
> **⏳ IN PROGRESS — Nav chrome redesign (July 2026), resume here next session.** The heavy dark-green (#244e43) 62px header + flat welded tab bar are being replaced app-wide with a **light airy header** (page-colored bg, brand green moved into soft-shadow white icon chips) + a **floating capsule tab bar** (iOS only; Android keeps the flat bar via a `Platform.OS` gate). Two new shared components: **`components/LightHeader.tsx`** (`LightHeader` + `HeaderChip` + `HEADER_ICON`) and **`components/FloatingTabBar.tsx`**. **No center "+" button** (add actions are contextual). **✅ DONE:** client `(tabs)/_layout.tsx` — applies to all 4 client main tabs (they share one navigator + header). **⏳ NEXT (after Vitek approves the TestFlight prototype):** migrate the client sub-screen headers (all-workouts, all-routines, availability, past-sessions, nutrition sub-screens — each still has its own OLD dark-green bar), then the trainer side (5-tab bar + `TrainerLogoButton`). Full detail in the `nav_redesign_light_header_floating_tabbar` memory. **The dark-green header specs immediately below (Client tab bar / Client tab header) describe the OLD design and are being superseded on migrated screens — do not treat them as current for the 4 client tabs.**
