...
> **⏳ IN PROGRESS — Nav chrome redesign (July 2026). AWAITING TESTFLIGHT FEEDBACK on the glass revision — resume here next session.** The heavy dark-green (#244e43) 62px header + flat welded tab bar are being replaced app-wide with a **GLASS light header** + a **floating GLASS capsule tab bar** (iOS only; Android keeps the flat bar via a `Platform.OS` gate). Two shared components: **`components/LightHeader.tsx`** and **`components/FloatingTabBar.tsx`**. **No center "+" button** (add actions are contextual).
>
...
> - **`LightHeader`** is now a frosted **`BlurView`** (expo-blur, intensity 40, tint light) + `rgba(250,249,247,0.55)` tone-over + hairline bottom border, **`position:absolute` OVER the content** so the page scrolls underneath it (WhatsApp see-through effect). Icons are **BARE — no white chip circles** — and larger: kettlebell **34**, VF **32**, back chevron **24**, rendered via the new **`HeaderIcon`** (bare button + optional green badge dot). The old `HeaderChip` circle was **removed**. Exports **`useHeaderHeight()`** (`insets.top + HEADER_ROW_HEIGHT(58)`); each of the 4 tab screens pads its scroll `contentContainerStyle.paddingTop` by it (+ `scrollIndicatorInsets.top`, + RefreshControl `progressViewOffset`).
> - **`FloatingTabBar`** is a frosted **`BlurView`** capsule (intensity 55, tint light, `rgba(255,255,255,0.55)` tone, borderRadius 28, soft shadow on a non-clipping outer wrapper). It **FLOATS over content** (host wrapper `height:0` → React Navigation reserves no space; pill absolutely pinned to bottom) so content scrolls under it and the glass actually reveals content. Exports **`useTabBarHeight()`** (`6 + PILL_HEIGHT(60) + max(insets.bottom,12)`); each tab screen pads its scroll `paddingBottom` by it. Icons size 24.
