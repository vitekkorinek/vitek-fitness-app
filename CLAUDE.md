...
> **⏳ Nav chrome redesign (July 2026) — tab bar + header DESIGN FINALIZED (settled with Vitek on a dev-client); migration in progress. RESUME AT NUTRITION next session.** The heavy dark-green (#244e43) 62px header + flat welded tab bar are being replaced app-wide with a **GLASS light header** (`components/LightHeader.tsx`) + a **floating GLASS capsule tab bar** (`components/FloatingTabBar.tsx`); iOS-tuned, Android keeps the flat bar via a `Platform.OS` gate. **No center "+" button** (add actions are contextual). **Two NATIVE modules are involved — `expo-glass-effect` (Liquid Glass bar bg) + `@react-native-masked-view/masked-view` (header blur fade) — so a FRESH NATIVE BUILD is required to see the final look.** JS-only tweaks (pill colours, sizing, gestures) hot-reload; the glass + masked blur do NOT.
>
> **`FloatingTabBar` (finalized):**
...
>
> **`LightHeader` (finalized):** WhatsApp-style glass header, `position:absolute` OVER the content (page scrolls under; each screen pads scroll `paddingTop` by `useHeaderHeight()` = `insets.top + HEADER_ROW_HEIGHT(58)`, + `scrollIndicatorInsets.top` + RefreshControl `progressViewOffset`). Bare icons (no chip circles): kettlebell 34, VF **26**, back chevron 24 — via `HeaderIcon` (bare button + optional green badge dot). **REAL progressive blur (the WhatsApp seamless edge):** a `@react-native-masked-view/masked-view` gradient MASK (opaque `black` behind the row, → `transparent` across a `FADE_ZONE(36)` strip below it) is applied to `BlurView(46) + tint rgba(250,249,247,0.55)`, so the blur STRENGTH itself ramps to zero — no hard edge, no step lines (stacked-blur bands were tried and rejected — they show "melted ice" lines). `HEADER_ICON = #244e43`. Screens use `StatusBar dark-content` (black clock over the light glass). Optional `overlay` slot = absolute element that never shifts the centered title (session timer, or `routine/[routineId]`'s `(i)` history button).
>
> **Rollout status (staged; Vitek tests each build):**
> - **✅ DONE:** client `(tabs)/_layout.tsx` + 4 tab screens; **+ 5 training sub-screens migrated to `LightHeader`**: `all-workouts`, `all-routines`, `availability`, `past-sessions`, `routine/[routineId]`. All keep `smartBack` + VF-home; old dark-green `SafeAreaView` headers + their styles deleted; root bg → `#faf9f7`; `StatusBar` → `dark-content`.
> - **⏳ NEXT SESSION — NUTRITION:** the 9 nutrition screens (`nutrition/index` food-log, `favourites`, `weekly`, `grocery-list`, `recommendations`, `tips`, `recipes`, `recipe/[id]`, `recipe/create`) still have OLD dark-green headers, AND the nutrition section's own nested 4-tab bar (`nutrition/_layout.tsx`) is still the flat bar → also migrate it to `FloatingTabBar`. The food-log **`index.tsx` is the most delicate** (PearIcon notification header, FAB popover using `useBottomTabBarHeight`, calendar, week strip) — migrate its header carefully. THEN the **trainer side** (5-tab bar + `TrainerLogoButton`).
