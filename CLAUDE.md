...
> **▶️ RESUME HERE — Do Mode "Session in progress" leave-dialog + frosted-glass confirm cards (July 2026, COMMITTED & PUSHED).** Both Do Mode files (`app/(client)/workout/[workoutId].tsx` + `app/(trainer)/client/[id]/workout/[workoutId].tsx`). Two things: (1) **The leave-session dialog** now has a `message` subtitle + **three real buttons** — **"Leave — keep it running"** (green, reworded from "Leave for now" to make the background-running behaviour clear), **"Discard session"** (red), **"Keep going"** (grey `confirmSecondaryBtn`; promoted from a faint `cancelText` link to a proper button). (2) **The shared centered `confirmModal` card** (and the free-session-name modal — same `confirmBox` style) is now a **frosted Liquid Glass panel** via a new module-level **`GlassPanel`** helper: real iOS-26 `GlassView glassEffectStyle="clear"` (gated by `isLiquidGlassAvailable()`) with a `BlurView intensity={18}` fallback, a uniform `rgba(255,255,255,0.5)` scrim for readability, borderless, `borderRadius:38`, soft shadow via `confirmBoxShadow`, lighter `centeredRoot` dim (0.38). **Buttons stayed FULL colour + white text** (translucent/tinted-glass buttons were tried and rejected as washed-out — they must match the solid pills used elsewhere). Full detail in §5 "Back button behavior" + the "Confirm-modal card = frosted Liquid Glass" note there. Typecheck baseline stays 29. Real glass only shows in an iOS-26 build. **All JS — hot-reload; `expo-glass-effect`/`expo-blur` already in the binary.**
>
...
**Confirm-modal card = frosted Liquid Glass (July 2026).** The shared centered `confirmModal` card in **both** Do Mode files (and the free-session-name edit modal — they share the `confirmBox` style) is a **`GlassPanel`** (module-level helper in each file): real iOS-26 **`GlassView glassEffectStyle="clear"`** (gated by `isLiquidGlassAvailable()` from `expo-glass-effect`), with a **`BlurView intensity={18}` fallback** off iOS 26. A **uniform light scrim** (`rgba(255,255,255,0.5)` absoluteFill inside the panel) keeps the same readable frosted level across the whole card while staying see-through — this replaced an earlier top-only text scrim + a mint-tinted-blur version. The card is **borderless** (no white rim — Vitek's call), `borderRadius:38`, wrapped in a `confirmBoxShadow` outer View (soft deep shadow; `confirmBox` itself is `overflow:'hidden'` so it can't hold the shadow). `centeredRoot` overlay dim was lightened to `rgba(0,0,0,0.38)`. **Buttons stay FULL colour with white text** (green/red/grey) to match how those pills look everywhere else in the app — translucent/tinted-glass buttons were tried and rejected as washed-out/incohesive. The `confirmMessage` subtitle is dark (`#33413b`, weight 500) so it reads on the glass. **Real Liquid Glass only renders in an iOS-26 build** (Expo Go shows the blur fallback).
