...
- **Client Training tab + modal:** **"Log any workout"/"Plan any workout"** (faded when nothing to log/plan) and **"Log workout from your routine"/"Plan workout from your routine"** (row HIDDEN entirely when `!activeRoutine`; carries the weekly "Done – … · Next – …" sub-line; today → routine detail directly). Day-aware title "Log training"/"Plan training" + ACCENT subtitle ("Today" / the date) + the 48h rest-hint lines (both variants) — full current spec in CLAUDE-screens.md "WeeklyGaugeCard" (day-contextual add/plan bullet).
