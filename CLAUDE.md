...
| **CLAUDE-infra.md** | `lib/supabase.ts`, `lib/sessionOutbox.ts`, `lib/otaUpdates.ts`, `lib/lastRoute.ts`, `lib/formDraft.ts`, `app/_layout.tsx`, or any screen's load/spinner logic — request deadlines, offline finish, OTA delivery, route restore, crash drafts |
...
- **Workout card layout (`WorkoutItem`) — ONE-LINE footer since July 26 2026 (Vitek's spec; the trainer twin in `client/[id]/all-workouts.tsx` was ported the same day and is identical):**
  ...
  - **Footer — a single row** at `paddingHorizontal:12, paddingVertical:4, gap:8`: `name` (15/700, `flexShrink:1`) · **bare last-done DATE in ACCENT** (12px; `—` muted when never done — the "Done " prefix was dropped, the green already says it) · `footerSpacer` (`flex:1`) · **routine name or "Standalone"** (11px `#999`, `flexShrink:1`) · ⋯. The spacer (rather than `flex:1` on the name) keeps the date glued to the name it describes and makes source+⋯ their own right-edge cluster.
...
- **Routine detail workout cards:** WEEKLY sections (July 26 2026 — replaced the cycle-aware NEXT UP / COMPLETED layout).
  ...
  - **Workout card layout (both client and trainer routine detail) — the app-wide short card since July 26:** `WorkoutPaperCover size="strip"` (84; was the default `full` 94) + a ONE-line footer `name · last-done in ACCENT (`—` when never; text is `relativeTime`, this screen's convention) ·· ⋯` at `paddingVertical:4`, so the card is 112 like every other cover card. The cycle-done `doneBadge` (18×18 ACCENT circle) moved from inline-beside-the-name onto the **cover's top-right**.
