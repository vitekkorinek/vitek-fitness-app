...
>
> **⬜ NEXT (trainer headers stage 2): the 5 trainer TAB screens** (clients/schedule/library/finance/account) still have dark-green `SafeAreaView` headers with `TrainerLogoButton`. Migrate them to `LightHeader` (likely **`solid`** too, to avoid the same ghosting; left slot = a `TrainerLogoButton`-equivalent glyph w/ notif badge + session logic; StatusBar dark-content; content pads `paddingTop: useHeaderHeight()` on top of the existing `paddingBottom: tabBarH`). Stage AFTER Vitek confirms the client-detail header in a build.
>
