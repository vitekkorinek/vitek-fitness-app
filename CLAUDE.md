...
>   - **⬜ Nutrition nested tabs** (`app/(client)/nutrition/_layout.tsx`, 4 tabs Food Log/Favourites/Weekly/Grocery) → convert to `NativeTabs` too. NOTE the food-log `index.tsx` uses `useBottomTabBarHeight()` for its FAB popover + `navigation.setOptions({tabBarStyle:{display:'none'}})` for selection mode — BOTH break under NativeTabs and need reworking (use `useTabBarHeight()` / a different hide mechanism).
>   - **⬜ Trainer tabs** (5-tab bar + `TrainerLogoButton`) → `NativeTabs`.
>   - **⬜ Retire `FloatingTabBar`** once all tab groups are on NativeTabs — but first move `useTabBarHeight()` out to a tiny shared module (still consumed by the 4 client tab screens) OR replace those paddings with the native inset.
> - **⏳ STILL PENDING — NUTRITION HEADERS (the ORIGINAL "resume at nutrition" task, not yet started):** the 7 real nutrition screens still have OLD dark-green 62px headers → migrate to `LightHeader`: `nutrition/index` (food-log, MOST delicate — PearIcon notif header, FAB popover, calendar, week strip), `favourites`, `weekly`, `grocery-list`, `recommendations`, `recipe/[id]`, `recipe/create`. (`tips.tsx` + `recipes.tsx` are just redirects — skip.)
