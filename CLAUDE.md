...
> **Since then — header tint unified.** The glass `LightHeader`'s mint green wash was rejected; there is now ONE neutral off-white tint on every screen (`plain` prop deleted, all call sites prop-free). Revert = flip `GREEN_TINT` in `components/LightHeader.tsx`. → CLAUDE-screens.md "Header tint".
