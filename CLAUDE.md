...
> **▶️ RESUME HERE (nav redesign — nutrition, July 2026).** Last session converted the **client nutrition tabs → `NativeTabs`** + migrated the top-level nutrition headers → glass `LightHeader`, then applied **round-2 build-feedback fixes** (content-too-low + hidden `+` buttons + weekly week-switcher — all committed & pushed). See the "🩹 BUILD-FEEDBACK FIXES (round 2)" bullet in the nav blockquote (~§2 UI Patterns, "Nav chrome redesign") for the how/why. **STILL OPEN from Vitek's round-2 feedback — DO THESE NEXT (need a fresh iOS-26 build to verify each):**
> 1. **Recipe open/create nav is likely BROKEN under NativeTabs.** Vitek: "you can't open recipes inside the recipes tab." `recipe/[id]` + `recipe/create` are currently `hidden` `NativeTabs.Trigger`s inside `nutrition/_layout.tsx`; `router.push('/(client)/nutrition/recipe/…')` probably doesn't navigate to a hidden dynamic trigger. **Recommended fix:** MOVE both files OUT of the nutrition tabs to stack routes under `(client)` (e.g. `app/(client)/recipe/[id].tsx` + `recipe/create.tsx`, parent `(client)/_layout` is a `Stack`), remove the two hidden triggers, update all nav paths `/(client)/nutrition/recipe/X` → `/(client)/recipe/X`, and in the moved files swap their `paddingBottom: tabBarH` back to `insets.bottom` (no tab bar once they're full-screen stack routes) + drop `useTabBarHeight`. (Client-only routes; trainer uses `/(trainer)/recipe-create`.) VERIFY the meals-list `+` (`openNewMeal`) still works — it opens the in-file `mealDetail` overlay, NOT a route, so it should be fine.
> 2. **Meal editor (`mealDetail` overlay in `favourites.tsx`, `md` styles) needs the frosted glass header + a redesign.** Vitek: "meals keeps the old header — it needs to stay frosted see-through, also redesign needed here." It currently keeps its own dark-green header (I gated `{!mealDetail && <LightHeader/>}`). The redesign scope is **open-ended — ASK Vitek in prose what he wants** before rebuilding it. At minimum give its header the frosted look. Note: `LightHeader`'s right slot is a fixed 44px icon area, so the meal editor's Name-edit / Save / Delete actions may need a custom frosted header rather than the stock `LightHeader`.
>
> After those, the nav redesign continues with the **trainer 5-tab bar → NativeTabs** (see the ⬜ items in the nav blockquote), then retiring `FloatingTabBar`.
...
>   - **⬜ Trainer tabs** (5-tab bar + `TrainerLogoButton`) → `NativeTabs`.
>   - **⬜ Retire `FloatingTabBar`** once all tab groups are on NativeTabs — but first move `useTabBarHeight()` out to a tiny shared module (still consumed by the client-tab + nutrition screens) OR replace those paddings with the native inset.
