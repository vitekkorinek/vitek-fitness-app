...
>
> **⬜ NEXT: the trainer 5-tab bar → `NativeTabs`** (`app/(trainer)/(tabs)/_layout.tsx` + `TrainerLogoButton`; see the ⬜ items in the nav blockquote), then retire `FloatingTabBar` (move `useTabBarHeight()` to a tiny shared module first).
>
