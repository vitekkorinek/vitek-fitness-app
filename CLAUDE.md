...
> **▶️ RESUME HERE — DESIGN LOCK-IN, ALL UNCOMMITTED (July 23 2026 evening): brand-DARK workout cards + Manrope locked app-wide, BOTH sides. Vitek will review on device and then EXPLICITLY ask to commit+push — do not push before he says so.**
...
> - **Trial machinery DELETED.** `lib/designTrial.ts` (zustand store + AsyncStorage + long-press `DesignTrialSheet` in the gauge) is gone → **`lib/appType.ts`**: plain `ft(weight)`/`fd(weight)` Manrope helpers (module functions, NOT hooks — append after base styles: `[s.name, fd(700)]`). `_layout.tsx` loads Manrope only; `@expo-google-fonts/space-grotesk` uninstalled; the 4 **Satoshi OTFs in `assets/fonts/` are now UNREFERENCED and untracked** — Vitek should delete or keep them before pushing.
