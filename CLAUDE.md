...
> **⏳ IN PROGRESS — Nav chrome redesign (July 2026). ⏸️ PAUSED, AWAITING A FRESH TESTFLIGHT BUILD — resume here next session.** The Liquid Glass tab-bar lens (below) uses the native **`expo-glass-effect`** module, which is NOT hot-patchable — Vitek must publish a **new native build** and wait for it to go live on TestFlight before he can see/test it. Next session: check his feedback on (1) the sliding Liquid-Glass lens magnify effect, (2) the more-see-through glass + darker icons, (3) the seamless (hairline-removed) header. THEN continue the migration (sub-screen headers, then trainer side). The heavy dark-green (#244e43) 62px header + flat welded tab bar are being replaced app-wide with a **GLASS light header** + a **floating GLASS capsule tab bar** (iOS only; Android keeps the flat bar via a `Platform.OS` gate). Two shared components: **`components/LightHeader.tsx`** and **`components/FloatingTabBar.tsx`**. **No center "+" button** (add actions are contextual).
>
> **Glass revision (July 2026, latest — after Vitek's prototype feedback: bottom bar "still on a panel / missing glass", clock must be black, header icons out of circles + kettlebell bigger, header see-through on scroll):**
> - **`LightHeader`** is now a frosted **`BlurView`** (expo-blur, intensity 40, tint light) + `rgba(250,249,247,0.55)` tone-over, **`position:absolute` OVER the content** so the page scrolls underneath it (WhatsApp see-through effect). **No bottom hairline** (removed July 2026 — Vitek wanted the seamless WhatsApp look where the header edge is invisible). Icons are **BARE — no white chip circles** — and larger: kettlebell **34**, VF **32**, back chevron **24**, rendered via the new **`HeaderIcon`** (bare button + optional green badge dot). The old `HeaderChip` circle was **removed**. Exports **`useHeaderHeight()`** (`insets.top + HEADER_ROW_HEIGHT(58)`); each of the 4 tab screens pads its scroll `contentContainerStyle.paddingTop` by it (+ `scrollIndicatorInsets.top`, + RefreshControl `progressViewOffset`).
> - **`FloatingTabBar`** is a floating glass capsule (borderRadius 28, soft shadow on a non-clipping outer wrapper). It **FLOATS over content** (host wrapper `height:0` → React Navigation reserves no space; pill absolutely pinned to bottom) so content scrolls under it and the glass reveals content. Exports **`useTabBarHeight()`** (`6 + PILL_HEIGHT(60) + bottomPad`, `bottomPad = max(insets.bottom-8, 8)` — sits a touch lower); each tab screen pads its scroll `paddingBottom` by it. Icons size 24; **inactive glyph/label = `#3a3a3c` dark grey** (prominent on glass), active = ACCENT.
>   - **Sliding selection LENS (mirrors the native iOS 26 / WhatsApp bottom-nav magnify effect):** a rounded selector springs (`Animated.spring translateX`, one-item wide, `itemWidth` from the pill's `onLayout` ÷ visible tabs) to the active tab, rendered ON TOP of the items. On **iOS 26+** (`isLiquidGlassAvailable()`, **`expo-glass-effect`** — installed July 2026, autolinked, no config plugin, **needs a fresh native build**) it's a real Apple **Liquid Glass** `GlassView` (`glassEffectStyle="clear" isInteractive`) that refracts/magnifies the icons it passes over; wrapped in a `GlassContainer spacing={20}` with a full-bleed `GlassView "regular"` bar background. On older iOS it degrades to a `BlurView` (intensity 65, `rgba(255,255,255,0.35)` tone — lowered for more see-through) + an animated grey `rgba(0,0,0,0.06)` fallback pill (no refraction). Lens `pointerEvents:'none'` so taps pass through. **Who sees Liquid Glass:** iOS 26+ only (≈ iPhone 11 / SE-2+, updated); older iPhones + not-updated + Android get the fallback / flat bar.
> - **Status bar:** all 4 client tab screens (+ overview) switched to `barStyle="dark-content"` so the clock is BLACK over the light glass.
> - **⚠️ UNVERIFIED on device:** the `FloatingTabBar` `height:0`-host float trick (RN reserving no space for the absolutely-pinned custom tabBar) passed typecheck but has NOT been run live yet. If the pill mis-positions or bottom content clips, revisit the host/padding math.
>
> **✅ DONE:** client `(tabs)/_layout.tsx` + the 4 tab screens (`train`/`schedule`/`progress`/`me`, plus `overview` status bar) — glass header + floating glass pill + dark status bar + content-scrolls-under. **⏳ NEXT (after Vitek approves the TestFlight prototype):** migrate the client sub-screen headers (all-workouts, all-routines, availability, past-sessions, nutrition sub-screens — each still has its own OLD dark-green bar), then the trainer side (5-tab bar + `TrainerLogoButton`). Full detail in the `nav_redesign_light_header_floating_tabbar` memory. **The dark-green header specs immediately below (Client tab bar / Client tab header) describe the OLD design and are being superseded on migrated screens — do not treat them as current for the 4 client tabs.**
