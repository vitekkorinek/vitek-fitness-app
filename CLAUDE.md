...
> **▶️ RESUME HERE — DESIGN LOCK-IN `99cb693` COMMITTED & PUSHED (July 23 2026): brand-DARK workout cards + Manrope app-wide, BOTH sides. Device-reviewed by Vitek: font renders everywhere (after the getter-patch fix below) and the bumped timer weights look good.**
> The July design trial ended: **dark home-tile cards + Manrope won** (clean-white and paper lost; full saga in memory `design_trial_clean_cards_fonts`). Locked in this working tree:
> - **NEWEST — `1debc48` COMMITTED & PUSHED (July 24 2026, TWO waves same day): client Training-tab RESTRUCTURE + "Workout card style" promoted to a PERMANENT setting — and in WAVE 2 taken APP-WIDE, BOTH SIDES (trainer entry point in Account → "TRAINER CARDS APPEARANCE"; every cover card incl. the week strips obeys it). Device-review-pending — nothing from July 24 has been seen on device yet.**
>   - **WAVE 2 (later July 24, this working tree): the setting went APP-WIDE.** Vitek's brief: trainer gets the same switch + the new card design, and the week-strip session cards lose their locked all-dark look on BOTH sides ("matching the rest of the workout cards"; Claude agreed — once every card has the contrast-footer anatomy, a seamless dark card reads as inconsistency, not emphasis; the "now" role is carried by position/width). Mechanics: **the `variantAware` opt-in prop was DELETED from `WorkoutPaperCover` — the cover follows the setting unconditionally**; every card paints frame + footer the OPPOSITE of the cover (white base + light lift shadow **y5/0.08/r12/el4 in BOTH styles**, dark overrides appended when `variant === 'light'`); **no cover card uses the 0.22 dark shadow spec any more** (client `darkCardStyles.outer` deleted). **Trainer entry point: Account tab → "TRAINER CARDS APPEARANCE" section, right after the CLIENT HOME BANNER section / before Business Details (Vitek's placement + wording; deliberately OUTSIDE the Save flow — it's device-local AsyncStorage, not `trainer_settings`)** — `BizRow` + the same swatch-miniature `BottomSheet` (`cardStyleSt` copy in `account.tsx`, new `t.account.appearance/workoutCardStyle/…` strings). Swept surfaces: client week-strip session cards (`WeeklyGaugeCard`); trainer Library workouts + templates (cover scrim pills — client pill / TEMPLATE badge — get quiet-ink `*OnLight` twins `rgba(0,0,0,0.06)`/`#8a8a86` on the white cover; same in add-workout); per-client All Workouts (`cardDark` → bg-only `frameDarkBg`); client-profile ALL 6 shapes (gallery minis, plan-picker cards, week-strip cards with `notYetBadge` light/dark twins, WorkoutRow + Recent Activity — those two are 70px **cover-CROP overlay cards** with no separate footer, so their texts follow the COVER: ink base, white when the cover is dark); add-workout picker; workout-picker; trainer routine detail (mirrors client). **Routine cards (`RoutineCard` sweep, trainer-profile routine section, all-routines) deliberately untouched — Vitek: "routines we will do after."** Typecheck 29.
>   - **Restructure (Vitek's brief: "too many dark green cards … one works well, many is overkill"):** (1) **ROUTINE section now FIRST under the week strip and UNBOXED** — new `RoutineReadout` replaces the v4 `ActiveRoutineCard` on this tab (card deleted with `arStyles`; revert ref `2dc5f6c`). Rationale: max one active routine → a card was a "gallery of one" and the tab's fourth dark slab; the routine is STATE, not a library item. Readout = name (16/700) + v4's **accent "Done – X · Next – Y" status line** (same string logic incl. "Cycle complete") + **light `ProgressRing` 48** right + the **routine-detail PROGRAM-ORDER strips row** (per-workout `CATEGORY_COLORS[cat].border`, `#888` fallback; done/next full opacity, pending 0.4; height 4 r2 gap 5). Whole readout taps → routine detail. **The ⋯/`RoutineDetailsSheet` flow was DROPPED from this tab** (component untouched — trainer side still uses it); the long-dead `RoutineQuickLookModal` + `qlStyles` copies were deleted too. **Section header says "Routine" and the WHOLE section is hidden when no active routine** (no placeholder — Vitek's call; `sectionStyles.noRoutine` deleted). (2) **WORKOUTS gallery second + sorted UP-NEXT:** not-done-this-week first, this-week ones fall to the back (kills the "same workout twice in a row" duplication with the strip session card; within groups the old recency order holds; `WorkoutCard.doneThisWeek` computed in `loadWorkoutsSection` from the REAL current week, never the viewed strip week).
>   - **Card style — PROMOTED to a permanent client-facing setting (Vitek's July 24 call, same day, after the dark-mode discussion: he likes both looks, testers should get the choice + feed back; when dark mode ships the THEME will pick the suitable default and this setting stays as the client's override).** Store **`useCardVariant` in `lib/cardVariant.ts`** is now **`'dark' | 'light'`** — **BOTH options are the trial's CONTRAST-FOOTER anatomies (Vitek's mid-build correction — the first cut wrongly kept the all-dark card and dropped the white-footer one): `'dark'` = DARK cover + WHITE footer** (the trial's `darkWhiteFooter`; default) **, `'light'` = WHITE cover + DARK footer** (the trial's `lightDarkFooter`: ink exercise list `rgba(0,0,0,0.68)` + **`CategoryCover` `variant='ink'`** — HEADER-green ink via the same explicit-fill/paperCrop machinery as `brand`, quieter levels 0.07 / 0.16–0.28 / border 0.12). **The all-dark seamless card is OFF the picker** *(wave 1 kept it on "locked surfaces" — week-strip card + trainer side; → superseded by WAVE 2 above: it's gone everywhere now)*. At every call site the footer is the OPPOSITE of the cover (`footerDark`/`galleryFooterDark` = `variant === 'light'`), and **both styles keep the base light lift shadow**. AsyncStorage key `workoutCardStyle` (the trial's `cardVariantExperiment` key is deliberately NOT migrated, just deleted — Vitek expects every device to start on the agreed default, not resurrect a stale trial pick). **Set in client Me tab → NEW "Appearance" section → "Workout card style" `EditableRow` → BottomSheet picker** with swatch miniatures of the two anatomies (`cardStyleSt` styles; new `t.clientMe.appearance/workoutCardStyle/…` strings). **The gauge LONG-PRESS + its trial sheet were DELETED** (the arc's `Pressable` reverted to a plain `View`). **Reacting surfaces in wave 1: Training-tab gallery minis, My Workouts, CLIENT ROUTINE DETAIL** (its `coverCardStyles` rebased to white + `cardDarkBg`/`textOnDark`/`subOnDark` overrides) — *the wave-1 "locked ALL-dark" list (week-strip card + trainer side) and the `variantAware` opt-in prop were both retired the same day; see WAVE 2 above.* **Dark mode = accepted ROADMAP item, not started** (app stays `userInterfaceStyle:"light"`; the white-cover card is the designated dark-mode card; full retheme is its own future milestone). Typecheck 29.
> - **`2dc5f6c` COMMITTED & PUSHED (July 23 late): client Training-tab feedback round (from Vitek's device screenshots; routine card iterated 4× on device).** (1) **My Workouts card tune-up** — `app/(client)/(tabs)/train/all-workouts.tsx`: `WorkoutItem` cover now `size="strip"` (84px — same as the Training-tab week-strip session cards; was default `full` 94px) + list gap 8→14 (dark cards need air). Trainer per-client All Workouts NOT matched yet. (2) **Client ROUTINES section: `RoutineCard` → `ActiveRoutineCard`, now v4 — Vitek's own sketch.** THREE rejected iterations the same evening: v1 compact WHITE row ("weak in comparison to the other cards"), v2 dark cover/footer card with the workout-name list as cover TEXT ("too big and simple"), v3 one-zone tile with the pill row under the name row (he disagreed and sketched v4). **v4 (current), full workout-card anatomy:** cover (84px, `DARK_CARD_GRADIENT`) holds the workouts as **free pills side by side** (workout NAME in its category's `pillBg`/`pillText`; NOTHING under them — deliberate) + **`ProgressRing` 48 `onDark` absolute right** (the silhouette slot); footer = name + **status line "Done – X · Next – Y"** (Done-part omitted while empty; "Cycle complete · Next – …" after a full cycle) + ⋯ (→ `RoutineDetailsSheet`). The "N workouts · X%" line is GONE (ring + pills carry it). `RoutineWorkoutItem` gained `name`; full anatomy in CLAUDE-screens.md "ROUTINES section". The tab's old `RoutineCard` copy + `rcStyles` + `formatRoutinePeriod` are DELETED (revert ref: `99cb693`). **The 3 remaining outlined `RoutineCard` copies (client `train/all-routines`, trainer `client/[id]/index` + `client/[id]/all-routines`) and the trainer profile's big-card section are UNTOUCHED — sweep them only after Vitek's device verdict.** **v4.1 device tweaks:** cover 84→**72** ("too tall still"), ring 48→**56** (stroke 3, text ×0.2 — "circle too small"), status line → **ACCENT green + `ft(600)`**, uncategorised-pill fallback → **mint `#E1F5EE`/ACCENT** (was white-alpha). **The white pill's real cause was DATA, not code: FIVE workouts literally named "Push" (4 in routines + 1 standalone) had `category=NULL` — which is also why the OLD RoutineCard's strip always rendered gray with a "—" label. Fixed directly in Supabase July 23 (service-role `PATCH workouts SET category='Push' WHERE name='Push' AND category IS NULL`, 5 rows).** Vitek ordered the commit right after the DB fix — treat v4.1 as accepted; the red Push pill itself is the only bit not explicitly seen on device yet. Typecheck 29.
> - **SAME EVENING, LATER — Manrope now TRULY app-wide via a global wrapper (the font sweep is DONE).** **`installAppFont()` in `lib/appType.ts`** redefines the **react-native INDEX getters** for `Text` + `TextInput` (`Object.defineProperty` — the index's object-literal accessors are configurable) with wrappers that append `ft(<declared fontWeight>)` to every style (weight→face map; 100–300→400, 900→800) — so every text and input in the app renders Manrope with NO per-site edits. Wrapper skips styles with an explicit `fontFamily` (SpaceMono, existing `ft`/`fd` sites) and nested `<Text>` spans without their own `fontWeight` (they must inherit the parent span's face). Installed from the NEW custom entry **`/index.ts`** (`package.json` `main` was `expo-router/entry`) via the side-effect module `lib/installAppFont.ts` — it MUST run before anything else initializes (RN's `Animated.Text` captures `<Text>` once at its module init, and import hoisting means a plain call in `index.ts` would run too late). Fail-safe: try/catch → app boots on system font if RN's module shape ever changes — now LOUD in dev (`[appFont] installed — Manrope wrapper active` / `install FAILED` console lines). **⚠️ LATE-EVENING FIX — the wrapper was silently broken on device at first:** v1 assigned `TextModule.default = wrapper`, which THROWS under SDK 54 Metro ("Cannot assign to property 'default' which has only a getter" — ES-module exports compile to getter-only live bindings) and the then-silent catch ate it every boot → system font everywhere except explicit `ft`/`fd` sites. NEVER patch a module's `.default`; patch the index getters. Related gotcha: entry-point changes (`package.json` `main`) do NOT hot-reload — restart the dev server. Fix verified July 23 in Expo Go sim (login screen Manrope incl. TextInput placeholders, install log present, deep-import deprecation warnings gone) and **CONFIRMED on Vitek's device same evening: "now i see the font everywhere, its working."** `Animated.Text` deep-requires Text and stays SF (accepted). Per-site `ft`/`fd` appends remain valid and idiomatic for NEW styling. Native chrome (NativeTabs labels, native headers) is not RN `<Text>` and stays SF — intentional. **Device-watch items:** invoice print-preview (`pvSt`) now renders Manrope while the printed PDF keeps its HTML font. ~~Do Mode timer digits not tabular~~ — WRONG: Manrope has `tnum`, so `fontVariant:['tabular-nums']` works (no jiggle, device-confirmed). Real device finding instead: timer digits read "too weak" in Manrope → bumped one weight step in BOTH Do Mode files (`combinedPillTimerText` + `restPillText` → `...fd(800)`, 46px `restTimer` clock → `...fd(500)`), spread into the style defs (explicit family → the global wrapper skips them). Device-review-pending.
> - **RoutineCard is no longer plain white:** dark-green outline — `borderWidth:1.5, borderColor:'#244e43'` (HEADER) on the inner `rcStyles.card` — in all 4 live copies: client `train/index.tsx` + `train/all-routines.tsx`, trainer `client/[id]/index.tsx` (inline hex — HEADER const sits below that StyleSheet, TDZ) + `client/[id]/all-routines.tsx`. Dead `app/(tabs)/all-routines.tsx` copy untouched. Deliberate brand border → lives on the "KEEP borders" list, not an exception violation. *(→ Superseded for `train/index.tsx` the same late evening: that copy was replaced by `ActiveRoutineRow` — see the NEWEST bullet; 3 outlined copies remain.)*
> - **Trial machinery DELETED.** `lib/designTrial.ts` (zustand store + AsyncStorage + long-press `DesignTrialSheet` in the gauge) is gone → **`lib/appType.ts`**: plain `ft(weight)`/`fd(weight)` Manrope helpers (module functions, NOT hooks — append after base styles: `[s.name, fd(700)]`). `_layout.tsx` loads Manrope only; `@expo-google-fonts/space-grotesk` uninstalled; the 4 **Satoshi OTFs were DELETED July 23** on Vitek's go-ahead (they were unreferenced and never committed — `assets/fonts/` keeps only SpaceMono).
> - **`WorkoutPaperCover` is always dark** ("paper" name kept — 12+ call sites): home-tile gradient inside the cover, white flow exercise list (NBSP-glued), category pill = the card's one color landmark; stair packer + clean/paper branches deleted. **`CategoryCover` variants are now `color`/`soft`/`muted`/`brand`** (`paper`+`ghost` and the per-category `paper`/`ink` fields deleted; `paperCrop` name kept, used by `brand`). Brand silhouette FINAL: all-white, body 0.09 / lit 0.20+0.33 / border 0.15 — quiet on purpose; muscle TINTING was tried at 3 strengths and rejected, do NOT re-propose.
> - **Dark frames/footers on EVERY cover-card surface, both sides:** client Training tab (gallery minis + week-strip session cards), My Workouts, client routine detail, trainer Library (workouts + templates), per-client All Workouts, trainer routine detail, add-workout picker (both tabs), workout-picker, client profile (all 6 card shapes). Pattern everywhere: frame+footer `DARK_CARD_FOOTER` (`#112820`), dark shadow spec y4/0.22/r10/el6, name `#fff` + `fd(700)`, subs `rgba(255,255,255,0.6)` + `ft(400)`, ⋯ `rgba(255,255,255,0.65)`; ACCENT badges/chips unchanged. **Deliberately still white:** RoutineCard + all-routines cards (white face, now with the dark-green outline — see bullet above), See-all dashed card, every non-cover card.
> - **Palette (same afternoon, also uncommitted): three-way hue SHUFFLE** — Pull→**gold** `#E0B12E`, Lower Body→**blue** `#3B7DC4`, Full Body→**green** `#3E9E5E` (Vitek: "green is the app color, full body makes sense there") — after Lower Body cycled green→petrol→moss, each colliding at pill size. **Arms hotter** `#EA6E15` (was collapsing into Mobility's bronze). Sets were transplanted whole between categories in BOTH canonical files.
> - **NOT done yet:** expo-font config-plugin embed (do at next native rebuild — Manrope still runtime-loads via `useFonts`); CLAUDE-screens.md not fully re-audited against the lock (gauge/week-strip sections may describe pre-lock styling). ~~App-wide font sweep~~ DONE via the global wrapper (top bullet).
> - Typecheck baseline 29 throughout.
>
> **Previously — `b388367` COMMITTED & PUSHED (July 2026): Do Mode card-notes rework, BOTH SIDES. Trainer built + device-reviewed across ~10 iterations; client mirrored same-day on Vitek's go-ahead. Typecheck baseline 29. Client side not yet device-reviewed.**
> **Card notes rework (Virtuagym-inspired, Vitek's brief with screenshot).** In BOTH Do Mode files (client mirror is role-flipped: inline input/pencils write CLIENT notes there, trainer notes read-only labelled TRAINER; footer/input hidden in view-only+preview) — full detail in **CLAUDE-domode.md** ("Collapsed set CHIPS", "Set-row note indicator", "`CardNoteFooter`", "Note EDITING"). Summary: collapsed cards show **set chips** (kg top / reps below, first 3 + `…`, green dot on a chip whose set has a note) + the **latest exercise note** under them; expanded set numbers get a **green dot** instead of the old bounce+darker treatment; the card footer is now **`CardNoteFooter`** — type a note directly on the card, pencil to edit, **"See more"** unfolds up to 5 previous notes inline and toggles to **"See less"**; a **"See all"** link (→ info panel) appears under the list only when >5 previous notes exist; notes are now **editable** everywhere delete exists (`editExerciseNote`/`editSetNote` + pencils in `ExerciseInfoModal`/`SetNoteModal`). Also from device review: `SetNoteModal` → full-width bottom sheet; header `floatIconBtn` scrim 0.22 → **0.45** (‹ and ⋯ vanished over bright banner photos); a **"Finish session" list-footer pill** (`finishFooterBtn`, white + ACCENT outline/text — filled was "too heavy", "N / M exercises done") while `isRunning` → same `handleFinish` confirm as the header pill; collapsed cards made **uniform** (chips always render `0 kg / 0×`, note line always shows w/ "No note" placeholder, 1-line ellipsis, circle `translateY:8` optical centering, name 16/600→**17/700**, list headers `bannerH + 10` so the first card's shadow isn't swallowed by the banner cap); **rest pill: green filled, bigger, draggable** (`restPillDrag` + 6px-slop PanResponder; overtime = red pill); **header FINISH removed while running** (timer-only pill — the bottom footer is the ONLY finish path; START unchanged); exercise title gets `letterSpacing:-0.4` (stopgap — **[[typography_brand_font_pending]]**: Vitek wants an app-wide brand-font pass later); **long-press the banner photo → full-screen uncropped peek** (`bannerPeek`, tap to close).
>
> **Previously — `13ec524` COMMITTED & PUSHED (July 2026). Typecheck baseline 29. Device-review-pending.**
> **Do Mode session-loss fixes + card/timer rework (from Vitek's first real trainer-side session, July 2026).** Five reported problems, all in the two Do Mode files (+ new `lib/sessionDraft.ts`). Full detail in **CLAUDE-domode.md** — "Session survival", "Rest timer", "Accordion", "Icon toolbar".
> - **A session could not be finished and its data was lost.** Root cause of the loss: `saveSession`'s failure path called `finishSession()` + `router.back()`, dumping the trainer out of Do Mode with everything still only in memory. Now: the update falls through to an **insert** if it errors or matches no row, and if nothing can be written at all the screen **stays put** with a "Couldn't save the session / Try again" modal. Nothing is cleared until a session id exists.
> - **Leaving Do Mode wiped weights + sets** (notes/photos survived because they hit the DB immediately). Now a **debounced AsyncStorage draft** mirrors the live `exercises` array, and `load()` **adopts today's open `in_progress` row** (correct start time, no duplicate session) and replays the draft over the fresh DB load.
> - **Cards are an accordion** — opening one closes the others (superset members excepted), so the fixed banner photo always follows the card you're in.
> - **Both sides now have the same card toolbar:** one bottom **icon toolbar** of 4 equal buttons — `play · info · camera · +` — with camera and + **dashed** (adds something) and + hard right. The old trainer top "Play video / Info" row + labelled "Add Set / Dropset · Add photo" row are deleted; the client's toolbar existed already but ran in the opposite order with mixed icon sizes, so it was flipped to match. The `+` menu gained a ✕ to back out of it.
> - **Rest timer is a slide-up panel and keeps running when dismissed** — a bottom-right pill shows the countdown (red in overtime); tap = reopen, ✕ = cancel. Only Stop/✕ cancel. Exercise Detail's own rest modal was left alone.
> - **Not verified on device yet.** Watch: the accordion's 140ms scroll delay, the pill's position vs the edit-mode action bar, and whether an adopted session ever resumes when Vitek expected a fresh START.
>
> **Previously — header tint unified.** The glass `LightHeader`'s mint green wash was rejected; there is now ONE neutral off-white tint on every screen (`plain` prop deleted, all call sites prop-free). Revert = flip `GREEN_TINT` in `components/LightHeader.tsx`. → CLAUDE-screens.md "Header tint".
