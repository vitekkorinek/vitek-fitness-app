...
> - **Trial machinery DELETED.** `lib/designTrial.ts` (zustand store + AsyncStorage + long-press `DesignTrialSheet` in the gauge) is gone → **`lib/appType.ts`**: plain `ft(weight)`/`fd(weight)` Manrope helpers (module functions, NOT hooks — append after base styles: `[s.name, fd(700)]`). `_layout.tsx` loads Manrope only; `@expo-google-fonts/space-grotesk` uninstalled; the 4 **Satoshi OTFs in `assets/fonts/` are UNREFERENCED and still untracked on disk** (deliberately left out of `99cb693`) — Vitek can delete or keep them anytime.
