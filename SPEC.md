...
- **Card-on-top visual:** white BG cap `height:26, borderTopLeftRadius:26, borderTopRightRadius:26` at `bottom:0` of `ListHeaderComponent`. First exercise card gets `marginTop:6` for breathing room (tightened from 12).
- **⚠️ WIP — FIXED HEADER (option 2), client Do Mode only, July 2026, behind a flag.** A module const **`FIXED_HEADER` (default `true`)** switches the client Do Mode header from the classic scroll-away hero to a **pinned banner** that stays fixed while the exercise cards scroll underneath it. The banner shows the **ACTIVE exercise's** photo + name + position count — the image/name/count follow **whichever exercise card you open** (`activeHeaderId`, set in `toggleExpand`; falls back to exercise 1). Layout: back `‹` top-left, ⋯ top-right, and bottom-left a stack of **overline** (`ADAM · SESSION 4 · 18 JUL 2026` = workout name + session + date, restored), **big title** (exercise name), **count** (`1 / 5`); the collapsible timer/stopwatch sits **bottom-right** (separated from ⋯). Photo source = `activeExercise.extraPhotoUrls[0] ?? thumbnailUrl ?? workout cover ?? CategoryCover watermark ?? gradient`. Banner height = `HEADER_MAX` (same as the old header). **The entire old scroll-away header + nav bar are preserved intact behind `!showFixedHeader`** (`showFixedHeader = FIXED_HEADER && !pastSession`); the main list's `ListHeaderComponent` becomes a `bannerH` spacer when fixed. **To revert: set `FIXED_HEADER = false` + rebuild** — nothing else changes. **Only affects live/main client Do Mode; past-session view and the entire trainer Do Mode still use the old header.** Known gaps (deferred): image swap is a hard cut (no crossfade), it's expansion-driven not scroll-driven, exercises without a photo fall back to a gradient, and the trainer side is not mirrored. Needs on-device testing of the full session flow.
