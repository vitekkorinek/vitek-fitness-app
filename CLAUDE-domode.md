...
- **Inside expanded:** bar/machine selector (if applicable) → "Sets" or "SUPERSET" label → KG/REPS/TOTAL header row → set rows → `addedSetsDivider` if applicable → **icon toolbar** → Start timer button → session photos → note footer
...
- **Running-rest pill** — shown when `restRunning && !restVisible && !isEditMode && kbHeight === 0`, bottom-right (`restPillWrap` / `restPill`, `insets.bottom + 16`): `timer` icon + `mm:ss` (red `#e53935` + `+mm:ss` in overtime) + hairline + ✕. Tapping the pill body reopens the panel; ✕ cancels.
