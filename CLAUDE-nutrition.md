...
**Landing page (default) — HUB layout since Aug 4 2026 (replaced the July 2×2 dark-tile grid).** A client reported the dark gradient cards read too heavy; Vitek's direction: the folders are only doorways to sub-tabs, so they shrink to **round badges** — "when it is already smaller and more stylized it will already feel lighter". Layout: **Recommendations as a filled circle in the CENTER, the 4 favourites orbiting it** as tinted circles — row (Recipes · Meals) / center / row (Foods · Days). One shared **`HubBadge`** component (`hub` styles): tinted 96px circle (106px + `itemCenter` width 250 for the center; sized up from 80/88 on Vitek's first device look — "maybe a bit bigger?"), 38px SF Symbol icon in the deep hue (42px center), **label (14px/600 TEXT) + count sub-line (12px MUTED) BELOW the circle**, spring scale 0.94 on press. `landingContent` = `padding:16, gap:22, flexGrow:1, justifyContent:'center'` (hub floats centered in the free space). `hub.row` = `space-between, paddingHorizontal:24`. The old `TileCard`/`FullWidthCard` components, `tc`/`fc` styles, `s.tileRow` and `s.sectionLabel` are **deleted**; the "from your trainer" split is now carried by the center sub-line `N items · from your trainer`.

```
Satellites (tint circle / icon colour):
Recipes — #f7ecd9 / #b06614 — book.closed.fill
Meals   — #e9edf8 / #33499a — fork.knife
Foods   — #e2f0f2 / #16697a — carrot.fill   (TEAL since Aug 4 — see below)
Days    — #f5e7f0 / #88376c — calendar

Center: Recommendations — filled HEADER #244e43, white pills.fill icon
```

**Palette rationale (Aug 4 2026):** hues stay as folder identity but move from the ground to accents — pale tint circle + deep-hue icon. **Recommendations = HEADER dark green** (the trainer's brand green; the old brown/taupe was rejected — "it makes no sense" once the satellites went light) and sits at nearly-peer size (88 vs 80) — the fill + center position carry its distinction, not bulk. **NOT the ACCENT green** — a big accent-green circle would read as an action button. **Foods moved off green to TEAL** (`#e2f0f2`/`#16697a`) because green now belongs to the center; Vitek picked teal over a tomato/coral option. Days' icon stays `calendar` (not `heart.fill`). **The header tint is the neutral off-white** — the mint green-tint was tried and rejected, app-wide (see CLAUDE-screens.md "Header tint"; revert = one flag in `LightHeader.tsx`).
