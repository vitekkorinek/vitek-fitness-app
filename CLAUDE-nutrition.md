...
## Trainer food library (Claude-seeded foods) + photos (July 2026)

Vitek's ask: rather than hand-create trainer foods one at a time in Library → Foods, have Claude generate accurate nutrition data for common foods and bulk-insert them into `trainer_foods` — they surface FIRST in client food search with the green VF badge (see "source icons" above; ranked 1100, always first — `lib/foodApi.ts`).

**What was seeded (LIVE in prod):** `trainer_foods` went from 1 → **249 rows** for trainer_id `28a75e2d-c81c-413c-ac2f-0d753c522eef`, inserted via Supabase MCP `execute_sql` (plain `INSERT`s, batched by group). Coverage: whole foods across all `food_groups` (fruit/veg/meat/fish/dairy/legume/grain/nut/fat), German staples (Brötchen, Vollkornbrötchen, Bratwurst, Wiener, Leberkäse, Schnitzel, Frikadelle, Quark, Magerquark, Skyr, Sauerkraut…), common prepared dishes (Pizza Margherita, Spaghetti Bolognese, Lasagne, Rührei, Omelett, Porridge, Sushi…), drinks (juices, Cola, beer, wine, coffee), condiments (honey, ketchup, mustard, soy sauce, mayo, pesto…) and sweets (chocolate, ice cream, biscuits…).
...
- **Natural portions** (Vitek's explicit request — "1 egg large/small, 1 apple, 1 cup of dry rice"): eggs = `1 large/medium/small`; fruit = `1 medium/small/large`; rice/pasta/oats/legumes = `1 cup` (dry-cup and cooked-cup gram weights DIFFER); bread = `1 slice`; nuts = `1 handful`; oils = `1 tbsp`/`1 tsp`; milk = `1 glass`; sausages/cheese slices = `1 slice`/`1 sausage`.
- **Dry vs cooked, raw vs cooked are SEPARATE rows, state labelled IN THE NAME** (English + German): e.g. `White rice (dry)`/`White rice (cooked)`, `Chicken breast (raw)`/`Chicken breast (cooked)`, `Lentils (dry)`/`Lentils (cooked)`. Deliberate so a client can't log the wrong state — keep it up when adding foods.

**Food photos = TheMealDB ingredient images.** Chosen over Pexels because it's a fixed 1-ingredient→1-image lookup (near-zero mismatches, no API key), and TheMealDB's image URLs are documented for direct display. Pattern: `https://www.themealdb.com/images/ingredients/{Name}.png/medium` (spaces → underscores; `/small`|`/medium`|`/large` sizes). `photo_url` points **directly at that CDN** for all but one food. **Almonds is the ONE self-hosted proof-of-concept** — uploaded into the public `trainer-foods` bucket at `<trainerId>/almonds.png`. **129/249 rows have a photo.** The ~120 without = prepared dishes, sausages, condiments, drinks (no TheMealDB ingredient image) + **6 whole foods TheMealDB genuinely lacks: Cauliflower, Chia seeds, Edamame, Grapes, Quark, Watermelon** — these render the clean `fork.knife` placeholder (fine; add by hand or another source later).
- **⚠️ SANDBOX GOTCHA (important next session):** in the Claude Code sandbox, **background curl/uploads silently fail** (foreground single curls work, but the harness auto-backgrounds multi-item curl loops, and those write nothing). That's why photos are **hotlinked via pure-SQL `UPDATE trainer_foods … FROM (VALUES …)`** rather than downloaded+self-hosted in bulk. To migrate to self-hosted (more robust; no dependency on TheMealDB uptime), run download+upload from a machine with working outbound network, or in small **foreground** chunks — the upload endpoint is `POST {SUPABASE_URL}/storage/v1/object/trainer-foods/<path>` with the service-role key + `x-upsert: true` (proven on almonds).
- **Credit:** TheMealDB's terms ask to credit them as the image source. Vitek agreed to a **tiny "food images via TheMealDB" line in an about/legal screen — NOT yet added (pending).** No text is on the images.

**Two code bugfixes shipped with this (LOCAL, hot-reload, NOT committed):**
- `app/(client)/nutrition/index.tsx` `handleAddFood`: a freshly-logged **trainer** food now pulls its thumbnail from `trainer_foods.photo_url` (the code previously only queried `food_cache`, where trainer foods don't live → the just-logged row showed the placeholder until a full reload). Mirrors the existing edit-path branch.
- `components/FoodSearchModal.tsx`: the portion-card image `contentFit` `cover` → **`contain`** (`s.portionImage`, height 160) so the full food photo shows instead of being cropped.
- Typecheck baseline stays **29**.

**Follow-up features (all BUILT July 2026 — DB live, code local/uncommitted):**
1. **VF badge = 3 TIERS by trust level — DONE.** (Redesigned after Vitek flagged that a green "accurate" badge on generic milk chocolate / unknown-brand chocolate spread is misleading.) A single **`trainer_foods.badge` column** (`text NOT NULL DEFAULT 'whole' CHECK IN ('whole','branded','generic')`) drives the VF colour in `SourceBadge` (the old `is_branded boolean` is kept but superseded — `badge` is authoritative). `TrainerFoodRow.badge` → `FoodResult.foodBadge` (replaced `isBranded`) via `trainerFoodToResult`. Colours: **🟢 whole `#244e43`** (whole foods + compositionally-fixed staples: fruit/veg/meat/fish/egg/plain dairy/legume/grain/nut/oil, honey, sugar, syrup, **dark chocolate 70%** since the % keeps it consistent), **🔴 branded `#e85d4a`** (named brand / fast food), **🟡 generic `#f5a623`** (generic processed/composite where brand+recipe are unknown and vary — an estimate). **Distribution 213 whole / 35 generic / 31 branded = 279.** Generic (yellow) set = sweets/snacks (Milk chocolate, Chocolate hazelnut spread, ice cream, chips, popcorn, biscuit, doughnut, muffin, gummy bears), sauces/condiments (ketchup, mustard, mayo, pesto, jam, soy sauce), cereals/supplement (muesli, granola, cornflakes, whey protein), composite dishes (pizza, bolognese, lasagne, fries, mashed potato, sushi, pancake, schnitzel, frikadelle, meatballs, falafel, baked beans) and processed sausages/deli (bratwurst, frankfurter, leberkäse, salami). Branded values are best-effort estimates (scanner is the accurate path), fast food per-100g + whole-item portion, no photos. **When adding a food: whole only if macros are source-independent; generic for a processed/composite estimate; branded only for a named product.** Badge renders in **two places**: the client search results (`SourceBadge` in `renderFoodRow`, FoodSearchModal) AND the **trainer Foods library card** (`FoodCard` in `app/(trainer)/(tabs)/library.tsx` — a coloured `VFIcon` size 12 next to the name via `foodStyles.nameRow`, coloured by `row.badge`) so the trainer sees each food's tier at a glance. The FoodsTab also has a **badge-tier filter** above the list — a horizontal pill row **All / Whole / Branded / Generic** with live counts (`foodStyles.filterRow`/`filterPill`/`filterDot`; state `badgeFilter`, combined with the name search in `filtered`), so the trainer can filter the library by colour. NOT shown on the logged `FoodLogRow`. The `FoodCreateModal` (add/edit food) does not yet have a badge-tier picker — new trainer-created foods default to `badge='whole'`; set it via SQL or add a picker later.
2. **Auto-saved scanned foods (no re-scan) — DONE.** New `scanned_foods` table (per-client, RLS `client_manage_own_scanned_foods`, `UNIQUE(client_id,source,source_id)`). `handleBarcodeScanned` **upserts the scan immediately** (before logging) then opens the portion picker with `scannedAt` set. `loadScanned()` merges the client's scanned foods (newest first) with custom foods into the **"My foods" tab** (`filteredMyFoods`). `SourceBadge` shows a **`barcode.viewfinder`** icon when `scannedAt` is set; the portion card shows **"Scanned on <date>"** (`fmtScannedDate`). Client's own scans only. Also flows into `recent_foods` on log. Enhancement not done: merge scanned foods into the All-tab name search (reachable now via My foods + recent).
3. **Recently-used foods on opening search — FIXED (July 2026).** FoodSearchModal "All" tab empty state renders "RECENTLY ADDED" (10 most recent from `recent_foods` by `last_used_at`, ~line 699) but it was showing nothing because `recent_foods` was silently failing on TWO schema bugs: `handleAddFood`'s upsert writes a `food_groups` column that didn't exist, AND the `recent_foods_source_check` CHECK only allowed `off|usda|manual` (rejected `trainer`/`custom`). Fixed via DB migrations (added `food_groups text[] NOT NULL DEFAULT '{}'`; widened source CHECK to `off|usda|manual|custom|trainer`) + backfilled the client's logged trainer foods. Now populates on every log. **Photos + longer list DONE (Vitek asked):** `loadRecent` fetches **20** (was 10) and enriches each row with a photo (`recentImg` map — trainer foods from `trainer_foods.photo_url`, off/usda from `food_cache.image_url`) passed as `imageUrl` in the render. Recency order kept (no frequency needed). Minor open item: recent rows still have grams-only portion (recent_foods has no `portions`).
