...
- All food is added via **one floating action button** (`styles.fab`): 56×56 ACCENT circle, white `plus` SF Symbol, `position:'absolute', right:20, bottom: insets.bottom + 2` (lowered again July 2026 — closer to the tab bar). Hidden while `loading` or in selection mode (`selectedIds.size > 0`), but **stays visible while the add-picker is open** (it is the ✕ that closes it). Scroll `paddingBottom` = `insets.bottom + 100` (non-selection) so the Water row clears the FAB.
- **FAB add-picker = anchored popover (July 2026, built).** This is the ONE deliberate exception to the app-wide "menus slide up from the bottom" convention — it opens as a **popover that scales out of the `+` FAB's bottom-right corner** (Virtuagym-style), NOT a `BottomSheet`. Implementation (all inline in `index.tsx`, no `Modal`):
  ...
  - **Rendered in a `Modal` so the dim covers the nav row too, without moving anything.** The nutrition screen sits *above* the tab bar (non-overlay), so hiding the tab bar expands the screen and **drops the FAB** — that looked wrong. Instead the popover (backdrop + card + ✕) lives in a transparent full-screen `Modal` (`animationType="none"`, `statusBarTranslucent`), whose backdrop dims the **whole** screen including the still-visible tab bar. Because the Modal frame is the physical screen (bottom = physical bottom, not tab-bar-top), the card + ✕ are offset by **`tabBarH` (`useBottomTabBarHeight()`)**: `bottom: tabBarH + insets.bottom + 42` (card) / `tabBarH + insets.bottom + 2` (✕) — landing them at the exact same spot as the resting `+` (`insets.bottom + 2` in screen coords), so nothing jumps. The resting `+` (screen-level, `styles.fab`) is rendered only when `!mealPickerVisible`; the Modal's ✕ takes over while open. Do NOT hide the tab bar for this picker.
...
**Meals list:** search bar (`recipeToolbar` + `searchBar` styles, shared with Recipes) + a `plus.circle.fill` **create button** (July 2026 — previously meals could ONLY be created by saving a combo from the Food Log's selection mode; now a meal can be built from scratch here) + **sort pills row**: **Newest** · **Oldest** · **A–Z** · **Z–A** — `mealSort` state `'newest' | 'oldest' | 'az' | 'za'`, default `'newest'`. `filteredMeals` IIFE applies name search then sort. Shows "No results / Try a different search" empty state when search finds nothing; the **empty state** (no meals at all) also has a "Create a meal" button. Cover photo (76×76) or indigo gradient placeholder (`#2e4288→#1d2d6a`) with fork.knife icon, meal name, item count, kcal/P/C/F macro row. Tap → full meal detail view. **Swipe left → red "Delete"** (`mc.swipeDelete`, `Swipeable`) → `confirmModal` "Delete meal?" → `deleteMeal` (July 2026 — the primary way to delete a meal).
- **New meal flow — opens the making page immediately, no pre-modal, no auto-prompt** (July 2026; Vitek: name it ON the page, remind on leave): `startNewMeal(ingredients)` (shared by the Meals "+", the empty-state "Create a meal" button, and Foods "Make meal") **inserts the `saved_meals` row right away** (`name:''`, `ingredients`, `visibility:'private'`), prepends it to `meals`, records `newMealDraftId`, and `openMealDetail`s it. It does **NOT** auto-open the name window — the **meal detail view IS the "meal-making screen"** (Add food / remove / rename / cover / notes; no separate meal-builder route; meals **auto-persist** — every edit saves live via `saveMealPatch`). Header: back chevron (left) → `closeMealDetail`; muted **"Name your meal"** placeholder + pencil (center, tap → `openNameSheet(false)`); empty right slot. At the **bottom, three stacked buttons** (`md.saveBtn`/`logBtn`/`deleteBtn`): **Save meal** (ACCENT filled → `closeMealDetail`, the explicit finish/save affordance since users didn't realise back = save — even though it's already persisted, it names-if-blank + closes), **Log this meal** (ACCENT outline → opens the Log Meal modal to copy it into the food diary), **Delete meal** (light-red `#fdecec` → `confirmDeleteMealDetail`).
  - **Name window = slide-up `BottomSheet` (`avoidKeyboard`), matching the Grocery "Add item" sheet** — NOT a centered modal. Titled **"Name your meal"** when blank (else "Rename meal"); Save disabled while blank. It has **no "Discard meal" button** (Vitek: meals are deleted by swiping in the gallery, not from here).
  - **Naming is enforced on leave, not up front:** `closeMealDetail` (detail back) checks the draft — an unnamed **empty** draft is discarded silently; an unnamed draft **with food** pops the name sheet as a reminder (`openNameSheet(true)`, `nameSheetLeave=true`). On that sheet, **Save** names it and leaves; **Cancel / overlay / drag** (`cancelNameSheet`) discards the still-unnamed draft and leaves. Opened via the header (`nameSheetLeave=false`), Cancel just closes. `newMealDraftId` is cleared once named (`saveRename`). A meal that somehow ends up nameless renders as italic "Unnamed meal" in the gallery.
