...
- **Tab structure:** 3 top-level tabs: Exercises | Workouts | Nutrition. Workouts has sub-tabs (Workouts / Templates). Nutrition has sub-tabs (**Recipes / Tips / Foods**).
- **⚠️ TIPS = ONE tab with TWO FOLDERS (Aug 2026).** The old **Recomm.** and **Tips** sub-tabs are gone: "Recomm." became **Tips**, and the hardcoded `DAILY_TIPS_LIB` "system tips" (14 Claude-written cards behind an eye-slash hide button) were **deleted outright** along with their tab, their `hiddenSystemIndices`/`settingsId`/`hideSystemTip` plumbing, and the plain-row `tipCard`/`systemTipCard` styles. Vitek: *"tips are the ones you made before … we can get rid off them now - the entire sub tab - and keep only my recommendations that will be called Tips now."* Inside the Tips tab, `TIP_FOLDERS` drives an **underline folder bar** — **Supplements** (`category='supplement'`) and **Healthy eating** (`category='tip'`). Underline, not another `GlassToggle`, because it is the THIRD nesting level and has to sit lighter than the glass sub-tab bar above it; it also matches the folder bar the client sees inside the Tips book, so both sides read the same.
  - `NutritionTipsTab` takes **no `category` prop** any more — it loads BOTH folders in one query (`trainer_id` only) and filters in render, so switching folders never refires the fetch. Save writes `category: folder`.
  - **Both folders get the full treatment:** cover photo, link URL, and the `RecommendationCard` photo card. The old split (photo cards + link for supplements, plain rows for tips) is gone. A photoless card falls back to its folder's gradient + a large watermark glyph (`recStyles.recFolderMark`). **⚠️ Labels, icons and colours live in `lib/tipFolders.ts` — `TIP_FOLDERS` / `FOLDER_GRAD` / `FOLDER_BAR` / `FOLDER_ICON` / `asFolder()`, imported by BOTH sides.** Never re-declare them in a screen; they were briefly local to `library.tsx` and the client carried inline copies of the same hex, which is the two-places-drift trap CLAUDE.md §8 flags for category colours.
  - **⚠️ The stored `category` values stay `'supplement'` / `'tip'`** even though the label is now "Healthy eating" — see the `nutrition_tips` note in CLAUDE.md §4 for why (old client builds).
- **Sub-tab `addTick` guard:** `nutAddTick` now feeds only the Tips tab, but keep the guard — use `useRef(addTick)` at component mount and only open the create modal when `addTick > addTickAtMount.current`. A tick never resets, so a bare `if (tick > 0)` fires on every later MOUNT rather than on the press.
...
- **Tip card tap:** opens a slide-up **`BottomSheet` detail panel** (July 2026 — was a centered detail sheet; not the edit modal). It has: cover photo/gradient, title, link URL (ACCENT color), body, an **"Edit Supplement"/"Edit Tip"** button → `close(() => openEdit(tip))`, "Delete" button → `close(() => setConfirmDelete(tip))` (capture `tip`/`recipe` in a local before `close()` since it nulls the source state). State: `recDetail: NutritionTip | null`. The recipe detail (`detail`) is likewise a slide-up `BottomSheet`. Edit/create forms and delete confirms stay centered.
- **Recipe + tip cards:** both `height: 120` in the `recStyles.card` style. Keep in sync — they share the same visual height, and the Tips tab uses `recStyles.listContent` for the same reason.
- **Tip search:** `recSearch` filters the CURRENT folder by title; the search bar renders in both folders.
- **`app/(client)/nutrition/recommendations.tsx` is UNREACHABLE dead code** — a hidden `NativeTabs.Trigger` with nothing routing to it; the client reaches tips through the Library book in `favourites.tsx`. It still says "Recommendations" and filters `category='supplement'` only. Left alone deliberately; delete it if you touch that folder.
...
**"Burned today" line (Aug 8 2026 — Apple Health, dormant until the HealthKit build):** directly under the ring, `burnedRow` (flame.fill 11 `#EF9F27` + 12px/600 `#3a7d6b` text): `Burned today ≈ N kcal`, de-DE grouped. **TOTAL kcal (resting + active) via `fetchMovementToday().totalKcal` — deliberately not active-only, because the GOAL above it is a total-for-the-day** (Vitek: total "is a better comparison"). **Display only — NEVER added to or subtracted from the goal** (*"nothing gets add to the goal set by me"*; "eat back your burned calories" math overfeeds because burn numbers are estimates). **Date-aware since the same day:** `fetchTotalKcalForDay(selectedDate)` refetches per selected day (effect keyed on `selectedDate`, stale-guarded), so a past day shows THAT day's burn next to that day's food — wording flips "today" / "that day"; future days show nothing. Gated on `appleHealthSupported` (optional-require, false on builds without the module) + the connected flag. **⚠️ The number will not match the watch vendor's own app** (Garmin said 2380 while the line said 1922) — the line sums only what has actually synced into Apple Health; that's expected, see the note in CLAUDE-screens.md Consistency. Phase-1 privacy: read on device, shown to the client, uploaded nowhere — see `lib/appleHealth.ts`.
...
- **NO Done button (Aug 2026)** — a day-tap already selects AND closes, so the full-width green pill only repeated the overlay tap / drag-down while reading as the sheet's primary action, which the days themselves are. Same rule as the Cancel sweep in CLAUDE.md §2: a sheet with no primary action ends at its last row. **This applies to the READ-ONLY panels here too** — the **pip detail** sheet (macro/micro rings) and the **recommendation detail** lost their Done/Close the same day. Claude first argued those were a terminus rather than a duplicate and kept them; Vitek overruled it — *"i disagree with the macro micro nutrients pips, done makes no sense there is the same as the calendar"* — and he is right: nothing in the sheet needs acknowledging, so the button is just a second dismiss. **What KEEPS its button here:** every text-entry sheet (grocery add, meal name/notes, recipe name/portions/instructions) — their Cancel pairs with a Confirm.
...
## Trainer food library (Claude-seeded foods) + photos

`trainer_foods` is bulk-seeded with ~280 common foods (whole foods, German staples, prepared dishes, drinks, condiments, sweets); they surface FIRST in client food search with the VF badge (ranked 1100, `lib/foodApi.ts`). The one-time seeding narrative, the sandbox curl gotcha, the shipped bugfixes, and the full badge/scanned/recent build story live in **CLAUDE-history.md** ("Trainer FOOD LIBRARY seeded"). The durable rules that govern **adding more foods** and how the tiers/photos/scanned/recent behave:

**Conventions baked into the data (do NOT break when adding more):**
- **All values per 100 g.** Columns: `calories_per_100g` (NOT NULL), `protein_g`, `carbs_g`, `fat_g`, `fiber_g`, `sugar_g`, `salt_g` (salt in **grams**, small values), `food_groups` (text[]), `portions` (jsonb `{label,grams}[]`, 100 g implicit/never stored). Egg-type protein foods use `food_groups = {meat}` (there is no 'egg' group). Condiments/sweets/drinks use `food_groups = {}` (empty) where no group fits.
- **`name_de` is ALWAYS filled** — search matches `name` OR `name_de` (`searchTrainerFoods` in `lib/foodApi.ts`), so German names must be present for German search to work.
- **Natural portions** (Vitek's explicit request): eggs = `1 large/medium/small`; fruit = `1 medium/small/large`; rice/pasta/oats/legumes = `1 cup` (dry-cup and cooked-cup gram weights DIFFER); bread = `1 slice`; nuts = `1 handful`; oils = `1 tbsp`/`1 tsp`; milk = `1 glass`; sausages/cheese slices = `1 slice`/`1 sausage`.
- **Dry vs cooked, raw vs cooked are SEPARATE rows, state labelled IN THE NAME** (English + German): e.g. `White rice (dry)`/`White rice (cooked)`, `Chicken breast (raw)`/`Chicken breast (cooked)`. Deliberate so a client can't log the wrong state.

**VF badge = 3 trust tiers.** `trainer_foods.badge` (`text NOT NULL DEFAULT 'whole' CHECK IN ('whole','branded','generic')`) drives the VF colour in `SourceBadge` (client search) and the trainer Foods-library `FoodCard`; `TrainerFoodRow.badge` → `FoodResult.foodBadge` via `trainerFoodToResult` (the old `is_branded` bool is superseded). Colours: **🟢 whole `#244e43`** (whole foods + compositionally-fixed staples — macros source-independent, incl. dark chocolate 70%), **🔴 branded `#e85d4a`** (named brand / fast food — best-effort estimates, no photos), **🟡 generic `#f5a623`** (generic processed/composite estimate, brand+recipe vary). **Rule when adding a food: `whole` only if macros are source-independent; `generic` for a processed/composite estimate; `branded` only for a named product.** The FoodsTab has a badge-tier filter (All/Whole/Branded/Generic with live counts, `badgeFilter`). `FoodCreateModal` has no badge picker yet — new trainer foods default `badge='whole'` (set others via SQL or add a picker later).

**Food photos = TheMealDB ingredient images** (fixed 1-ingredient→1-image lookup, no API key). `photo_url` points directly at the CDN: `https://www.themealdb.com/images/ingredients/{Name}.png/medium` (spaces → underscores; sizes `/small`|`/medium`|`/large`). Foods TheMealDB lacks (prepared dishes, sausages, condiments, drinks + a few whole foods e.g. Cauliflower/Chia/Edamame/Grapes/Quark/Watermelon) show the clean `fork.knife` placeholder — add by hand or another source later. Almonds is a self-hosted proof-of-concept in the public `trainer-foods` bucket (`<trainerId>/almonds.png`). **⚠️ Sandbox gotcha:** background curl/upload loops silently write nothing in the Claude Code sandbox (foreground single curls work) — that's why photos are hotlinked via pure-SQL `UPDATE … FROM (VALUES …)` rather than downloaded+self-hosted. Do bulk self-hosting from a machine with real outbound network (endpoint `POST {SUPABASE_URL}/storage/v1/object/trainer-foods/<path>`, service-role key, `x-upsert: true`). **Credit pending:** a tiny "food images via TheMealDB" line in an about/legal screen is agreed but NOT yet added; no text is on the images.

**Scanned + recent foods (behaviour):** barcode scans upsert into `scanned_foods` (per-client, RLS `client_manage_own_scanned_foods`, `UNIQUE(client_id,source,source_id)`) the moment a barcode resolves (before logging), surface in the **"My foods"** tab (`filteredMyFoods`) with a `barcode.viewfinder` badge + "Scanned on <date>", and flow into `recent_foods` on log. `recent_foods` requires `food_groups text[] NOT NULL DEFAULT '{}'` + source CHECK `off|usda|manual|custom|trainer` (both were the silent-write-fail bugs — fixed via migration + backfill). The "All" tab empty state renders **RECENTLY ADDED** (20 rows by `last_used_at`, photo-enriched via the `recentImg` map — trainer photos from `trainer_foods.photo_url`, off/usda from `food_cache.image_url`). Open item: recent trainer rows are grams-only (no `portions`).
...
## Client **Library** tab (`app/(client)/nutrition/favourites.tsx`)

**⚠️ The tab is called LIBRARY since Aug 5 2026 — the file/route is still `favourites`.** Vitek: *"part of that tab there is also Recommendations from the trainer and recipes from the trainer so the favorites idea actually doesn't hold up."* Two of the five folders are the trainer's, so "Favourites" was never true; Library covers all five. **Only the user-facing name changed** — the route stays `/(client)/nutrition/favourites` (`?tab=` deep links, the `NativeTabs.Trigger name`, the Food Log's insert-a-day link and a stored `lastRoute` all point at it), so **do not rename the file without sweeping every call site**. Renamed strings: the `NativeTabs` label (icon `book.closed` → `book.fill` when selected, was `heart`/`heart.fill`), the landing `headerTitle`, the client-home Nutrition tile bullet, and the Food Log's "Add a day from your Library" picker row + the "saved to your Library" toasts. **The word "favourite" survives where it is the ACTION, not the place** — the week-strip ♥ that saves a day, the selection-bar "Favourite" button, `favourite_foods` / `favourite_days`, the Days card's plum heart badge, FoodSearchModal's Favourites filter tab.

**Landing page (default) — THE BOOK, v4 (Aug 5 2026, evening).** The centre is a hardback that opens on landing; the five folders fly out of the gutter as page-shaped badges. `BookHub` + `HubOrb` in `favourites.tsx`.

**⚠️ Three versions were rejected on device before this one, and NONE of them failed on the rotation.** They failed because parts of the book were **faded in instead of uncovered**. Do not reintroduce any of it:
- the right page and the spine had `opacity: anim`, so half the book *materialised* out of nothing;
- the shut cover was CENTRED by a static offset, so it sat half a page from where those pages then appeared — the book also jumped sideways as it opened;
- and because that offset ran the spine down the MIDDLE of the shut cover, the spine had to be hidden, losing the one mark that says "book" when it is shut.

**v4 anatomy — one moving part, and nothing ever coplanar with it.** Stage `132×94` (`BK_W 66 × BK_H 94`), gutter at the midpoint, placed so the gutter is the hub centre. Three layers, in this z-order:
- **the static right half** — the green board with the paper inset in it. Carries the book's only shadow. Its opacity is a **hard step** (`[0,0.03,0.0301,1]→[0,0,1,1]`): it is *hidden* while the cover lies flat on top of it, and appears the instant the cover starts to move. See the flicker rule below — this is not a fade, it is a one-frame swap between two identical pictures. (Consequence: the SHUT book has no shadow. Accepted.)
- **the cover, outside face** — the green board with the cream **"N"**, the binding sliver, and the inner hairline frame. `left: BK_W`, so it sits exactly on the static half while shut.
- **the cover, inside face** — the same plate from the other side: a green board with the LEFT page inset. `left: 0`.

**The two faces are SIBLINGS, each with its own rotation — NOT a face pair nested inside one rotating leaf.** Nested was built and rejected: it flickered *"like it would have electric shock"* and the **N stayed visible after the book had opened**, which is one bug — two coplanar layers inside a 3D-rotated parent are ordered by depth rather than document order, so they z-fight, and RN's backface culling does not survive the nesting. As siblings they are the standard RN flip-card. Visibility is controlled **only** by a hard opacity step at `anim` 0.5 (invisible, because the plate is edge-on there); **`backfaceVisibility` is deliberately NOT used** — one mechanism, one source of truth. The back is simply the front + 180°, each hinged on the gutter from its own side, so mid-swing they are the same physical plate:

```
front   rotateY '0deg'   → '-180deg'    left: BK_W   hinge = its LEFT edge
back    rotateY '180deg' → '0deg'       left: 0      hinge = its RIGHT edge
```

- **⚠️ NEGATIVE, and that is load-bearing.** A POSITIVE `rotateY` pushes the free edge AWAY from the viewer, so the cover swings out the BACK of the book and comes round the far side (*"it seems like it actually swing from down up like from behind the book to the front… i have to see the book open towards me"*). Negative brings the free edge forward, over the top, and lays it down on the left — same landing spot, opposite arc. Closing runs it back, so the cover lifts toward you and lays down to the right, which is also how you shut a real book.
- **⚠️ Hinging is `translate → rotate → translate`, NOT `transformOrigin`.** Moving the origin also drags the perspective vanishing point onto the hinge, which foreshortens brutally exactly where the gutter is. **Mind the signs:** rotating about a point `p` measured from the CENTRE is `T(p) · R · T(-p)`, so the FIRST translate carries `p` and it is **negative for a left-edge hinge** — front `[-H, rot, +H]`, back `[+H, rot, -H]`, where `H = BK_W/2`. Inverted, each face pivots on its FAR edge and swings clear of the book before flipping in mid-air: *"the page that opens and close completely disattach from the book and then flips"*. The symptom of a wrong hinge sign is "it detaches", not "it rotates oddly".
- **the paper is inset inside the green boards** — `top/bottom 4`, `3` on the outer edge, and **`3.5` at the gutter**. The outer inset makes the cover overhang it like a hardback instead of reading as a flat card; the gutter inset is what DRAWS the gutter (2 × 3.5 = 7pt of green between the pages), so no strip has to be laid across the middle.

**⚠️ THE FLICKER RULE — nothing may be exactly coplanar with the rotating cover.** Every "electric shock" shimmer this component produced came from a flat layer sitting at the same place and the same depth as the cover at **rot 0**, where a 3D-transformed layer and a flat one get ordered per frame and trade places ~60×/sec. Vitek's report located it precisely — *"when it opens the middle part of the book does it and when it closes the N does it"* — both of which are rot 0. Four things follow, and all of them must stay:
1. **the static right half is hidden while the cover lies flat on it.** The cover is an identical green board, so the picture never changes and there is nothing underneath to fight. (This was the N shimmering as the book shut.)
2. **no strip is laid across the gutter** — it would be coplanar with the cover's hinge edge. (This was "the middle part" shimmering as it opened.) The gutter comes from the boards' own inset edges instead.
3. **no shadow on anything that rotates** — a shadow on a 3D layer is its own shimmer source. The only shadow is on the static half.
4. **anything hung on the stepped board must be fully UNDER the shut cover, or it will blink.** The 3.5px binding sliver was first a child of that board, so it blinked in just after the cover began to move and out again just before it landed — reported as *"a tiny tribble"* opening and the side *"moving a bit in"* closing. It is a child of the **cover front face** now, so it is present exactly as long as the cover is.

So it **opens right→left and shuts left→right** — a normal book — and the open spread is centred on the gutter, which is the hub centre. **The SHUT book therefore sits ~31px right of centre, deliberately:** the gutter stays put and the book grows leftward, the way a book on a table does, and it keeps the gutter exactly where the pages fly from. An animated stage translate would centre it but would drag the gutter away from the badges' fly-out origin — that trade was made knowingly.

**Choreography — the two directions are NOT mirror images, and every number here is a device fix.**

| | cover | pages |
|---|---|---|
| **open** | starts at 0, `timing` **760ms**, `Easing.bezier(0.34, 1.02, 0.44, 1)` | start at **+320ms**, `timing` **820ms**, `Easing.bezier(0.2, 1.25, 0.4, 1)`, staggered 45ms |
| **close** | starts at **+380ms**, `timing` **640ms**, `Easing.bezier(0.42, 0, 0.25, 1)` | start at 0, `spring` `speed:20, bounciness:0`, staggered 38ms, reversed order |

- **The cover swing is timed, not sprung.** A spring fast enough not to feel mushy swung it in ~450ms — *"super fast"*.
- **Opening keeps a y > 1 control point** so the cover flops onto the page instead of stopping dead. **Closing must NOT overshoot:** past flat the free edge tips away from the viewer and perspective pulls that side inward, so a shut cover visibly shrinks a hair at the end.
- **The pages' fly-out is the mock's own curve**, kept because Vitek asked for it by name — *"i like how in the mock up you made it slows down and kind of settle down"*. The `1.25` IS the settle: each page overshoots its spot slightly and eases back. It replaced a `speed:14, bounciness:9` spring that read as too abrupt.
- **The 380ms the cover waits to shut is longer than it looks** and was 170ms first — *"the book closes before the pages get in"*. The LAST page only sets off after four staggers, and its own spring has to finish on top of that. Bounce is dropped on the way in: a page wobbling as it lands inside a shut book reads wrong, and every ms saved is a ms the cover is not sitting open waiting.
- **Auto-opens 180ms after focus** via its own `useFocusEffect`, which also resets everything on blur — Library is a persistent native tab, so without the reset it would stay open forever.

**Badges (`HubOrb`):** page-shaped `72×88`, bound (radius 5) on the left edge and rounded (16) on the free edge, a hairline binding rule, icon 32, label + count below. Each unspins from `±14°` as it flies. **Two rules that cost device rounds:** the book renders BEFORE the badges so every page draws ABOVE it, and each badge's `opacity` ramps over the first quarter of its flight (`[0,0.25,1]→[0,1,1]`, not the raw `anim`) — underneath, with opacity tracking the spring, a page only becomes visible once it has cleared the book's outline, which reads as coming from BEHIND it. `pointerEvents` is gated on `open` so collapsed badges are never tappable.

**Orbit — a fixed pentagon, absolutely positioned, no runtime measurement.** Container `View height:460` inside the centred `landingContent`; `hubW = Dimensions width − 32`, `hubCy = 240`. Offsets are badge-centre relative to the hub centre and **the fly-in delta is simply the negated offset**: reco `(0,−178)` w190 (wide so the label never truncates) · recipes/meals `(∓sideX,−64)` where `sideX = min(126, hubW/2−58)` · foods/days `(∓86,+112)`, the four at **w112**. **Pushed out Aug 5** — at the old `(∓118,−52)` / `(∓78,+100)` / w124 the ring cleared the open book by only **16pt at the sides and 9pt at the bottom** (*"the pages could go a bit aways from the open book no?"*); it is ~21pt on both now. ⚠️ The side pair **cannot just move further out** — their labels already reach the screen edge, which is why the label width had to come down 124 → 112 to buy the x. Ring stays even at ~138 / ~141pt; Tips is deliberately further at 178, alone at the top of the pentagon. Originally tuned over six device rounds on 4 Aug. `left = hubW/2 + x − w/2`, `top = hubCy + y − ORB_H/2`.

**Labels (Aug 5 2026):** **Vitek's Tips** · **Recipes** · **Saved Meals** · **Saved Foods** · **Saved Days** — the trainer's book is signed, the client's three say whose they are, and Recipes is left bare because it is both of theirs. Tips' sub-line is the bare count, since the title already says who it is from. Two-word labels are why the badges grew to 78×94 and the book shrank to `62×88` in the same round.

**Icons:** `lightbulb.fill` (Tips) · `frying.pan.fill` (Recipes — moved off `book.closed.fill`, a book badge orbiting a book hub read as a second hub) · `fork.knife` · `carrot.fill` · `calendar`.

**Rejected en route (do not re-propose):** a bookcase of three shelves with ghost-book props and the author on each spine (built and OTA'd Aug 5 evening, rejected — *"i want to come back to the idea of the book, it also matched the icon we have in the navigation panel"*); books orbiting a book (*"the books cant come out of a book"*); a 3+2 grid of front covers (*"makes no sense without the 6th one"*); standing spines without props (titles have to be sideways AND abbreviated at ~58px each).

**The previous landing — AUTO-OPENING BOOK HUB (Aug 5 2026, morning; replaced the same day)** (the heart hub below is the direct predecessor; everything about the ORBIT — pentagon offsets, stagger, auto-open-on-focus, collapse-on-blur — is unchanged, only the centre object and the badge SHAPE were replaced). **`BookHub`** — **a NORMAL book, spine on the left.** Shut, it is a dark-green cover (`HEADER`) with a cream `#f0ece0` **"N"** (for Nutrition), centred. It **opens right→left** — the N swings around the spine and lies down as the left page — and **shuts left→right**, the cover coming back over the pages. ⚠️ **Both directions are Vitek's explicit spec** (*"it has to open from right to left so we have N and that goes towards left opening a book, as all book open … first the pages are going in, the book closes after from left to right as book always do"*), and they were got backwards twice before this: v1 mirrored the whole book, and v2 then tried to make the cover travel left→right in BOTH directions, which a single hinge cannot do and which cost a whole leaf-swapping `mode` machine. **The plain physical book needs none of it — one leaf, one hinge, one interpolation.** Do not reintroduce a second leaf or a mode.
- **Geometry:** the spine is a fixed line at the stage midpoint. Exactly ONE leaf moves — the front cover, positioned to the RIGHT of the spine (`left: BK_W`) and hinged ON it (`transformOrigin:'left center'`, `perspective:900`). It is a flip card: **cover face forward, cream left page on the back** (`backFace` = its own `transform:[{rotateY:'180deg'}]`, so at net 360° its content is unmirrored; both faces `backfaceVisibility:'hidden'`). `anim` interpolates `'0deg'→'180deg'` — 0 = shut, 1 = open. The still RIGHT page and the 6px spine are `opacity: anim` (they must not exist while the book is shut).
- **The shut cover is centred by a STATIC offset on the cover face** (`left: -BK_W/2`), never by sliding the stage — nothing animated is allowed to fight the rotation. The rotation foreshortens that offset to zero at 90°, which is exactly where the two faces swap, so the handover never jumps.
- Both faces ALSO hard-cut via `coverOpacity`/`pageOpacity` (a step at `anim` 0.5) — belt and braces if `backfaceVisibility` ever misbehaves; at that angle the leaf is edge-on so the cut is invisible either way.
- `BK_W 66 × BK_H 94` — the open spread is 132 wide, and that is what caps the book's size: the side orbs sit at `x = ±118` with a 72-wide page, so their inner edge is at 82 and anything wider than ~140 crowds them.
- **Choreography: the book opens FIRST, the pages fly out 150ms later; closing reverses it** (pages in first, cover shuts 170ms after) — `animateHub` branches on `to` for this, it is not a symmetric stagger.
- **The book renders BEFORE the orbs so every page draws ABOVE it**, and each page's `opacity` ramps over the first quarter of its flight (`[0,0.25,1]→[0,1,1]`, not the raw `anim`). Both are required for Vitek's *"the things should fly out of the inside of the book"* — underneath the book, and with opacity tracking the spring, a page only became visible once it had cleared the book's outline, which reads as coming **from behind** it.
- **Page badges replace the round orbs**: `hub.page` is `72×88`, bound (radius 5) on the LEFT edge and rounded (radius 16) on the free outer edge — a leaf torn out of the book — plus a hairline `pageRule` at `left:9` for the binding and a soft `0.05` shadow so it reads as paper. Icon 32 (was 35 in the 88px circle). **Height is still 88, so `top: hubCy + o.y - ORB_H/2` is the same math the circles used** — the pentagon did not move. Each page also **unspins** from `±14°` (`spin`, from the sign of the orbit offset; reco `-8°`) as it flies out, so the pages tumble rather than slide.
- **The Recipes orb icon moved off `book.closed.fill` → `frying.pan.fill`** — with a book as the centrepiece, a book badge inside it read as a second hub. Meals/Foods/Days/Reco icons are unchanged.

**The heart hub it replaced (Aug 4 2026, kept for the palette + orbit rationale, which still hold):**

**AUTO-BLOOM HEART HUB (three steps: the July 2×2 dark-tile grid → a static hub with Recommendations center → the tap-to-expand heart orbit → auto-bloom, next day).** A client reported the dark gradient cards read too heavy; Vitek's direction: the folders are only doorways to sub-tabs, so they shrink to **round badges**. He first picked tap-to-expand ("i want a") over auto-bloom, then reversed on Aug 4 ("want to go with (b) so there is not need to tap on it"): **a big ACCENT `heart.fill` sits in the center and the FIVE folders spring into orbit ON THEIR OWN ~180ms after the tab lands** (staggered 45ms springs, orbs fly from the heart to their laid-out spots, scale 0.15→1 + fade) **while the heart morphs to a white heart with green outline** (white `heart.fill` + ACCENT `heart` outline stacked, cross-faded — deliberately the same morph language as the Food Log FAB's + → ✕). **Tapping the heart still collapses it** (reverse-staggered) and re-expands — kept even though "i dont think people will do it a lot". **Recommendations moved INTO the orbit (12 o'clock), still the one HEADER-green filled circle but the SAME 96px as the others** — the heart owns the center now (device round 2: "the heart is not in the center … really orbiting … [reco] can be the same size as the others").
- **The orbit is a FIXED PENTAGON, absolutely positioned — not flex rows** (the flex-column version put the heart below true center because the top orbs pushed it down, and its `measureInWindow` delta plumbing went with it). Container = a plain `View height:460` inside the centered `landingContent`; heart center at `(hubW/2, hubCy=240)` where `hubW = Dimensions width − 32`. `hubOrbit` offsets (circle-center relative to heart center): reco `(0,−178)` w190 (wide so "Recommendations" + its sub never truncate — they did in round 1 at w124) · recipes/meals `(∓sideX,−52)` where `sideX = min(118, hubW/2−62)` (clamps for narrow phones) · foods/days `(∓78,+100)` (**evened Aug 4 rounds 4-5** — `(∓80,+142)` put the bottom pair ~163pt from the heart vs the upper pair's ~129 ("the bottom ones look more apart?"); the ∓96 correction then spread them too far from EACH OTHER, pulled back to ∓78 which keeps the ring even at ~127pt). **Orb circles are 88 (icon 35); the heart glyph is 80 (button 116)** — round 5 shrank the circles from 96 "one little tic" for more space; rounds 3-5 were all "make it more airy". **When OPEN the heart = ORB_BG fill + HEADER outline at `weight="light"`** (round 6 thinned it from medium — "less thick"; round 5 darkened it from ACCENT) so it sits fully in the badge family; closed stays the filled ACCENT heart. **Fly-in delta = simply the negated offset** — no runtime measurement.
- **`HubOrb` component** (`hub` styles): tinted 88px circle, 35px SF Symbol, label (14px/600 TEXT) + count sub-line (12px MUTED) below, press-spring 0.94; `left/top/width/delta` come from `orbPlacement(key)`. Wrapper animates `opacity` + `translate`/`scale(0.15→1)` from its `anim`; **`pointerEvents` gated on `open`** so invisible orbs are never tappable.
- **State:** `hubOpen` + `heartAnim` + `orbAnims[5]` (order: reco, recipes, meals, foods, days). `animateHub(to)` staggers open in that order (45ms) and closes in reverse; `toggleHub` (the heart tap) just calls it. A dedicated `useFocusEffect` schedules `animateHub(1)` 180ms after focus (**the auto-bloom** — the delay lets the tab transition settle so the bloom reads as an event) and its cleanup clears the timer + collapses everything on tab blur, so **every fresh visit blooms again from the lone heart** (Favourites is a persistent native tab — without the reset it would stay open forever). Returning from a sub-list within the tab keeps it open (the focus-time bloom already ran).
- The muted "Tap the heart" hint under the heart was **deleted** with auto-bloom (it existed only as first-run discoverability for the tap gate; a user who deliberately collapses the hub has just used the toggle and needs no hint).
- `landingContent` = `padding:16, gap:22, flexGrow:1, justifyContent:'center'`; `hub.row` = `space-between, paddingHorizontal:24`. The old `TileCard`/`FullWidthCard` components, `tc`/`fc` styles, `s.tileRow`/`s.sectionLabel` are **deleted**; "from your trainer" is the Reco orb's sub-line.

```
ALL FIVE pages: tint #e9efec (ORB_BG) / icon HEADER #244e43 (ORB_ICON)
Icons: pills.fill · frying.pan.fill · fork.knife · carrot.fill · calendar
Book: HEADER cover + '#f0ece0' N · '#f6f5f0' pages · HEADER spine  (the centre is the only dark mark)
```
*(Pre-Aug 5: the centre was an ACCENT `heart.fill` that morphed to an ORB_BG fill + HEADER `heart` outline at `weight="light"` when open, and Recipes used `book.closed.fill`.)*

**Palette rationale — MONOCHROME GREEN since the same evening (Aug 4 2026, Vitek: "lets go b").** The orbs went through per-folder hues first (amber/navy/teal/plum tints + a filled HEADER-green Recommendations; during that phase Foods moved off green to teal, and Reco's dark fill had itself replaced a rejected brown/taupe) before Vitek noticed the deeper problem: **the colourful macro/micro pips in the Food Log already own the "colourful circles with icons" language**, so a colourful hub dressed two unrelated systems identically — and once Reco joined the orbit its dark fill "made no sense". Resolution: **every orb is the same pale HEADER-green tint with a HEADER icon — colour = nutrient data, green = navigation; the centre object is the only strong mark** (the ACCENT heart then, the HEADER-green book cover since Aug 5 — the rule survived the swap unchanged). Two roads mocked and declined: all-dark-green-GRADIENT circles (five dark blobs = the heaviness complaint reborn) and colourful-orbs-with-neutral-grey-Reco. Recommendations is distinguished only by position (12 o'clock) + its "· from your trainer" sub-line. **The meal/recipe LIST washes stay coloured** (blue/amber) — different surface; the hue link to the hub was a bonus, not the system. Days' icon stays `calendar` (not `heart.fill`). **The header tint is the neutral off-white** — the mint green-tint was tried and rejected, app-wide (see CLAUDE-screens.md "Header tint"; revert = one flag in `LightHeader.tsx`).
...
**Recipes list:** search bar + `plus.circle.fill` create button (→ `startNewRecipe`, see below). **Filter pills row** (below search bar, above recipe cards): **All** · **Mine** · **Vitek's** — `RecipeFilter` type `'all' | 'mine' | 'trainer'`. All = no filter; Mine = `created_by === clientId`; Vitek's = `created_by_role === 'trainer'`. Filter applied before query-string filter. Pill styles: `filterPill` (border 1.5px BORDER, borderRadius 100) / `filterPillActive` (ACCENT bg+border) / `filterPillText` / `filterPillTextActive`. Default: All. Recipe cover cards (`rc` styles, 130px; cover photo keeps the dark bottom scrim + white text; **no-photo fallback is the LIBRARY PAGE PAPER since Aug 5 2026** — flat `#e9efec` (the same paper as the hub badges and the book's own pages) + a `frying.pan.fill` watermark 72px at `PAPER_MARK rgba(36,78,67,0.16)` `{right:-4,bottom:-10}`, name in `HEADER`, sub in `PAPER_SUB rgba(36,78,67,0.52)`, no scrim. A photo-less card reads as a page out of the book instead of a coloured tile. Replaced the Aug 4 light amber wash, which had itself replaced a dark one; a nameless recipe still renders italic **"Untitled recipe"**, now in `PAPER_SUB`. Tap a card → the recipe **detail** screen (`/(client)/recipe/${id}`). **The recipe detail (`recipe/[id].tsx`), the recipe editor (`recipe/create.tsx`) and the meal editor (`meal/[id].tsx`) use the SAME paper fallback** — `PAPER_BG '#e9efec'` + the icon at `rgba(36,78,67,0.30)`, declared locally in each of the three files. Vitek asked for it explicitly: *"make sure you change it also in the recipe and meal builder"*, which reverses the earlier "a single hero cover isn't the stacked-list problem" call. Their dark amber / dark navy hero fallbacks are gone and `expo-linear-gradient` is no longer imported by any of the three. **The TRAINER side matches** (`library.tsx`, same day, on his *"yes change it in the trainer library too"*): the recipe **card** and the recipe **detail** cover both use `PAPER_BG` + `frying.pan.fill`, replacing a `#1a5c4a→#24ac88` green gradient and a 🍽 emoji. ⚠️ Two things the trainer card needed that the client's did not — its dark scrim is now rendered **only over a photo** (over paper it just dirties it), and the card's white name/sub and the non-own `person.fill` source badge flip to `HEADER` / `PAPER_SUB` when there is no photo, or they vanish. *(The tip card's gradient is a folder identity, not a photo fallback, so it was never part of the paper-fallback sweep — and since Aug 2026 both folders are green, see `lib/tipFolders.ts`.)*

**Meals list:** search bar (`recipeToolbar` + `searchBar` styles, shared with Recipes) + a `plus.circle.fill` **create button** (July 2026 — previously meals could ONLY be created by saving a combo from the Food Log's selection mode; now a meal can be built from scratch here). **No sort pills** (July 2026 — the Newest/Oldest/A–Z/Z–A `mealSort` row was removed; Vitek: "alphabetically by default and that's enough"). `filteredMeals` IIFE applies the name search then always sorts **alphabetically** (`name.localeCompare`). Shows "No results / Try a different search" empty state when search finds nothing; the **empty state** (no meals at all) also has a "Create a meal" button. **Meal cards are COVER-IMAGE cards** (`mc` styles — `height 130, borderRadius 14`; photo cards keep the dark bottom scrim + white text; **no-photo fallback is the LIBRARY PAGE PAPER since Aug 5 2026** — flat `#e9efec` + a `fork.knife` watermark 72px at `PAPER_MARK`, `HEADER` name, `PAPER_SUB` sub, no scrim; "Unnamed meal" italic in `PAPER_SUB`. Same treatment and same reason as the recipe cards — the whole gallery is pages from one library. The Aug 4 light-navy wash it replaced had itself replaced a dark navy one, which was the client complaint that started the redesign — the SAME card shape as the recipe cards (`rc`) and workout cover cards, so the gallery is uniform (July 2026 — were horizontal thumb+text rows). **The light-wash-on-light-card gradient is allowed here** (unlike workout covers' flat-white rule) because it's a colour tint ending on a *deeper* tint, not a white wash — but it is device-review-pending. Tap → the meal editor screen (`/(client)/meal/${id}`). **Swipe left → red "Delete"** (`mc.swipeDelete`, `Swipeable`) → `confirmModal` "Delete meal?" → `deleteMeal` (the primary way to delete a meal).
...
**Tips list** (`view === 'recommendations'`, header title **"Tips"** since Aug 5 2026 — the book on the top shelf is called Tips). **The "a book called Tips opens on a Supplements tab" wrinkle is RESOLVED (Aug 2026):** the inner switcher now reads **Supplements · Healthy eating**, so Tips is the container and neither folder repeats its name. Nothing was filtered out to get there — the earlier proposal (drop the switcher, query `category='tip'` only) would have HIDDEN the supplement content and was correctly never done. The internal `view` key, the `?tab=recommendations` deep link, the `recommTab` state values and the stored categories all keep their old names. Fetches `nutrition_tips` where `category IN ['supplement','tip'] AND is_published=true`, ordered newest first. `Recommendation` interface includes `category: 'supplement' | 'tip'`. **Tab switcher** (underline style, matching Progress screen): `recommTab` state `'supplement' | 'tip'`, default `'supplement'`. Tab bar: `recommTabBar` (`flexDirection:'row', justifyContent:'center', gap:32, paddingTop:20, paddingBottom:6`); active item has `borderBottomWidth:2, borderBottomColor:ACCENT`; text 20px/500 `#bbb` inactive, 20px/600 TEXT active. **The tab bar is INSIDE the ScrollView** (folds under the header — see the search-fold rule above). List filtered by `recommTab`. **⚠️ TILES, not strip cards (Aug 2026).** A 2-per-line grid (`recommGrid` / `recommTile`, `width = (screenW − 32 − 10)/2`, height 104, radius 16): cover photo if set (+ a top-to-bottom dark scrim, **photo only** — over the flat gradient it just muds it), else the folder gradient, plus a big low-alpha folder glyph bleeding off the top-right corner, and the **title alone** bottom-left (14px/700 white, up to 3 lines). **No chevron and no body preview** — Vitek: *"the cards have the usual card with chevron to the right, i think because there are only pop up windows we could approach it differently? perhaps only small tiles with titles … so its like buttons?"* He is right about the cause: a chevron promises a screen push and every one of these opens a popup, so the row shape was describing something that does not happen. **Because the preview went, the TITLES were shortened to carry the tile on their own** ("Fats - And Which Oil To Cook With" → **Fats & Cooking Oils**, "Fat Loss - Never Eat Below Your BMR" → **Fat Loss**, its sibling → **Cardio & Fat Loss** so the two stay distinguishable). Keep new tip titles ≤ ~22 characters. **⚠️ ONE GREEN FOR BOTH FOLDERS** — deep forest `#3a7d6b→#244e43`; the folders are told apart by their ICON alone (`pills.fill` / `leaf.fill`), on top of the tab you are standing in. Two steps to get there: supplements were amber `#c87820→#e89840` (*"the yellow for suplements doesnt work i think - the green for healthy eating is on brand"*), then briefly a second, brighter green `#2fb894→#1c7f66` on his own suggestion, which he dropped on sight — *"i think we dont need two different greens. so yea make it the same color as healthy eating."* **Do not re-propose a per-folder hue:** the tab already names the folder and the icon already names the card type, so colour was a third signal doing no work and diluting the brand green. All of it comes from `lib/tipFolders.ts`, whose per-folder Record shape survives so a future divergence stays a one-line change. *(The trainer's own list keeps the full-width photo cards WITH the body preview — that is his authoring view, where finding a card matters more than tapping one.)* Tap → centered GLASS modal (`width:'85%', overflow:'hidden'`): gradient top 100px (`FOLDER_GRAD`) + a 4px rule in the folder's light stop (`FOLDER_BAR`) + a **scrolling** body (title 17px/700 + body 14px, `maxHeight: min(460, screenH*0.48)`, `showsVerticalScrollIndicator`) + a **link BUTTON** + "Close" link. Client read-only. State: `selectedRecomm: Recommendation | null`.
  - **⚠️ This popup's card wrapper is a plain `View` with the dismiss `Pressable` as an absolute-fill SIBLING** — see the glass-popup scroll rule in CLAUDE.md §2. Restoring the usual `<Pressable onPress={()=>{}}>` wrapper silently kills the scroll.
  - **The link sits BELOW the ScrollView, not inside it** — it is a bordered row with an `arrow.up.right.square.fill` glyph, labelled with `prettyLink(url)` from `lib/utils.ts` (bare host; a supplement's product URL is a 300-char tracking link) and opening the FULL url via `Linking.openURL`. Inside the scroll it would need scrolling to find. The trainer's detail sheet uses the same `prettyLink` + `Linking.openURL` treatment inline under the title.
