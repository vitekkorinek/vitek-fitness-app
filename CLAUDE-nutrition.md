...
- **Frosted glass `LightHeader`** (title = meal name, else "New meal"; back → `handleBack`). The **cover is a hero that scrolls UNDER the header** → the scroll uses `contentInsetAdjustmentBehavior="never"`. `StatusBar dark-content`.
...
- Order: **Cover** (220px, hero, tap → `ImagePicker` → `meal-covers` bucket, camera badge) → **NAME row** (tappable card → name `BottomSheet`; the name is now a BODY field, NOT baked into the header) → **kcal/P/C/F** card (Protein `#378ADD` / Carbs `#EF9F27` / Fat `#D85A30`) → **INGREDIENTS** (🍏/image 52×52 + name + amount·kcal·macros; swipe-left red "Remove"; tap → amount-edit centered modal with live preview + "Remove from meal") + **Add food** (opens `FoodSearchModal`) → **NOTES** (tappable → notes `BottomSheet`) → **SHARE WITH** pills (No one/My trainer/My clients, save on tap) → **Log this meal** (ACCENT filled, PRIMARY; disabled when no ingredients → opens Log modal: date + meal-category pills) → **Delete meal** (light-red). Toast on log. (Dropped the separate "Save meal" button — back finishes; it already auto-saves.)
