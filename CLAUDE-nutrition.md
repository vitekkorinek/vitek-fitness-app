...
**Landing page (default) — TAP-TO-EXPAND HEART HUB since Aug 4 2026 (two steps: the July 2×2 dark-tile grid → a static hub with Recommendations center → the heart orbit, same day).** A client reported the dark gradient cards read too heavy; Vitek's direction: the folders are only doorways to sub-tabs, so they shrink to **round badges**. Final form (his pick "i want a" after feeling an interactive mock; the static hub was device-approved but "something is not sitting right"): **a big ACCENT `heart.fill` (96px) sits alone in the center; tapping it springs the FIVE folders into orbit** (staggered 45ms springs, orbs fly from the heart to their laid-out spots, scale 0.15→1 + fade) **while the heart morphs to a white heart with green outline** (white `heart.fill` + ACCENT `heart` outline stacked, cross-faded — deliberately the same morph language as the Food Log FAB's + → ✕). Tap again = reverse-staggered collapse. **Recommendations moved INTO the orbit (12 o'clock), still the one HEADER-green filled circle but the SAME 96px as the others** — the heart owns the center now (device round 2: "the heart is not in the center … really orbiting … [reco] can be the same size as the others").
...
- **State:** `hubOpen` + `heartAnim` + `orbAnims[5]` (order: reco, recipes, meals, foods, days). `toggleHub` staggers open in that order (45ms) and closes in reverse. A dedicated `useFocusEffect` cleanup collapses everything on tab blur, so **every fresh visit to the tab starts at the lone heart** (Favourites is a persistent native tab — without the reset it would stay open forever). Returning from a sub-list within the tab keeps it open (no extra tap).
- A muted "Tap the heart" hint sits under the heart, fading out as it opens (first-run discoverability — the known cost of the gate; Claude flagged it, Vitek chose the heart anyway).
